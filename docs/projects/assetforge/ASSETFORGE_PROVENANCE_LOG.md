...
| 2026-07-03 | Unreal Engine 5.8 | Controlled cairn repaired GLB import validation | `Saved/AssetForgeResearch/benchmarks/outputs/unreal/import_validation/assetforge_cairn_repaired_unreal_import_validation_20260703-080443.json` | Complete. Repaired `20k` and `50k` GLBs imported as one static mesh each, then temporary Unreal content was deleted. Raw imports had one LOD, one fallback material slot, one convex collision entry, and about `1 m` bounds. This validates importability only; scale normalization, material slots, tangent cleanup, LODs, UV/material metadata, and collision policy remain required AssetForge post-process work. |
...
| 2026-07-03 | Unreal Engine 5.8 | Controlled cairn import-validation report | `Saved/AssetForgeResearch/benchmarks/outputs/unreal/import_validation/assetforge_cairn_repaired_unreal_import_validation_20260703-080443.json` | Research-only report confirming the repaired `20k` and `50k` GLBs import in Unreal and that temporary imported `uasset` files were deleted after measurement. |
