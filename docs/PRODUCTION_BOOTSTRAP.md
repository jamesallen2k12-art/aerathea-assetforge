...
- `BP_GNM_OGR_BattlefieldEncounter_A01` has a native class, compiled Blueprint asset, dependency validation, phase-state plan, actor slots, collision volumes, variables, events, optional-branch gates, shieldwall impact forwarding, startup placement, and assigned pylon/caster/Manticore branch actors.
- `BP_OGR_CrudeTekPylon_A01` and `BP_CRE_ManticoreInterrupt_A01` are native-backed first-pass wrapper Blueprints placed in startup and wired to the encounter coordinator for phase review.
- `VFX_GNM_AetherShieldWall_A01` has a final Niagara art-pass handoff and build/import status ready. The current helper mesh/material-state implementation remains the review fallback; empty target `NS_GNM_AetherShieldWall_A01` exists, and the native shieldwall actor owns a Niagara component that pushes the documented `User.*` parameters. Final authored emitters remain pending.
