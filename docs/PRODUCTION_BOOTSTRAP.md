...
- Blender `.blend` sources and FBX exports exist for the first-slice target dummy, portal arch, modular ground tile, Mekgineer crate, ten Gnome armory child assets, the palisade module set, the first gnome base body, the first gryphon blockout, the first Infernal adult/Lesser lifecycle scale review set, and `SM_INF_CullingTrialFloor_A01`.
