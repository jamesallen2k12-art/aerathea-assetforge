...
- `KIT_GNM_OGR_RivalryEncounter_A01` first child `BP_GNM_HeavyMekShieldwall_A01` has an approved first-pass native Blueprintable actor, Blueprint asset, `SM_GNM_AetherShieldProjector_A01`, strengthened segmented `SM_GNM_AetherShieldWall_A01`, `VFX_GNM_AetherShieldWall_A01`, blue shield material state instances, LOD0-LOD3, required projector sockets, and startup review placement.
...
3. Decide whether the next Gnome/Ogre rivalry child should be a heavy Mek child, Ogre warrior wave, cairn gate, or Manticore interrupt package.
