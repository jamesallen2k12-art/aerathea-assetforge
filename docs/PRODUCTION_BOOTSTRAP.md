...
- `BP_GNM_OGR_BattlefieldEncounter_A01` has a native class, compiled Blueprint asset, dependency validation, phase-state plan, actor slots, collision volumes, variables, events, optional-branch gates, and shieldwall impact forwarding. Startup placement is deferred until optional branch imports can be assigned cleanly.
- `VFX_GNM_AetherShieldWall_A01` has a final Niagara art-pass handoff and build/import status ready. The current helper mesh/material-state implementation remains the review fallback; `NS_GNM_AetherShieldWall_A01` is not authored, but the native shieldwall actor now owns a Niagara component and pushes the documented `User.*` parameters.
