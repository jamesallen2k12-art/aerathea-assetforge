...
| `KIT_GNM_OGR_RivalryEncounter_A01` | Source intake and production package ready for 21 Gnome-vs-Ogre encounter images plus 4 added heavy Mek variants; first child `BP_GNM_HeavyMekShieldwall_A01` has approved first-pass projector mesh, strengthened VFX helper/material states, Blueprint actor, sockets, and startup placement; `SK_OGR_Teknomancer_A01` first-pass class-fit DCC/Unreal review import is complete | Review the combined startup capture, then choose the next heavy Mek child or Ogre warrior/cairn gate route |
...
2. Review the updated startup scene with `SM_MKG_GrappleHook_A01`, `BP_GNM_HeavyMekShieldwall_A01`, and `SK_OGR_Teknomancer_A01` visible together.
3. Choose whether the next Gnome/Ogre rivalry child should be a heavy Mek child, Ogre warrior wave, cairn gate, or Manticore interrupt package.
...
10. Continue Ogre final-art planning from the imported `SK_OGR_Base_A01` and `SK_OGR_Teknomancer_A01` review slices, then choose Warrior, Shaman, or Necromancer as the next class package.
