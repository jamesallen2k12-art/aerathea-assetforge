...
| `KIT_GNM_OGR_RivalryEncounter_A01` | Source intake and production package ready for 21 Gnome-vs-Ogre encounter images plus 4 added heavy Mek variants; approved sequence is complete through shieldwall Blueprint/VFX contract polish, Ogre Teknomancer, Heavy Mek, Ogre Warrior, Ogre cairn gate, Ogre Shaman/Necromancer packages, base/interrupt Manticore planning, crude Tek pylon planning, encounter assembly Blueprint planning, and final shieldwall Niagara art-pass handoff | Review which approved package should move from handoff to DCC/Unreal implementation |
| `VFX_GNM_AetherShieldWall_A01` | First-pass helper mesh/material-state implementation, native shieldwall VFX parameter contract, and native Niagara component hook exist; final Niagara art-pass handoff ready; `NS_GNM_AetherShieldWall_A01` not authored | Author the final Niagara system and validate it against the existing native `User.*` parameter contract |
| `KIT_OGR_Teknomancy_A01` | Production package and child intake ready; `SM_OGR_CrudeTekPylon_A01` child package ready; other Teknomancy children pending | Choose the next Teknomancy child after the rivalry encounter assembly needs are clear |
...
| `SM_OGR_CrudeTekPylon_A01` | Production package, modeling handoff, build/import status, source reference review, material plan, socket plan, collision plan, LOD plan, and future Blueprint state notes ready | Review and approve before DCC static mesh build |
| `BP_GNM_OGR_BattlefieldEncounter_A01` | Native-backed encounter coordinator implemented; Blueprint asset created and compiled; exposes dependency validation, phase-state controls, actor slots, trigger volumes, optional-branch gates, and shieldwall impact/pylon/caster/Manticore event hooks | Place in startup only after pylon, caster, and Manticore branch imports can be assigned cleanly |
...
| `SK_OGR_Base_A01` | First-pass male/female Ogre body sources generated, imported to Unreal, material instances and LOD0-LOD3 generated, sockets/physics/ABP placeholders created, startup validation passing | Replace blockout geometry with approved sculpt/retopo, UVs, authored textures, tuned physics, and animation set |
| `SK_OGR_Teknomancer_A01` | First-pass Ogre Teknomancer class-fit source generated, imported to Unreal, material instances and LOD0-LOD3 generated, sockets/physics/ABP placeholder created, startup review actor placed | Review combined startup capture, then resolve final shared Ogre skeleton fit, sculpt/retopo, UVs/textures, tuned physics, and animation set |
...
| `SK_OGR_Shaman_A01` | Production package, modeling handoff, build/import status, source reference review, material plan, LOD plan, collision plan, sockets, and animation hooks ready | Review and approve before DCC class-fit build |
| `SK_OGR_Necromancer_A01` | Production package, modeling handoff, build/import status, source reference review, material plan, LOD plan, collision plan, sockets, and animation hooks ready | Review and approve before DCC class-fit build |
...
| `SK_CRE_Manticore_A01` | Base production package, source concept intake, modeling handoff, build/import status, skeleton/proportion plan, material plan, LOD plan, collision plan, and sockets ready; source references visually inspected | Review and approve the base direction before DCC build |
| `SK_CRE_Manticore_Interrupt_A01` | Encounter-variant production package, modeling handoff, and build/import status ready; source references visually inspected; now references base `SK_CRE_Manticore_A01` | Wait for base approval/import before DCC build of the interrupt variant |
...
| Ogres | War-created, heavily muscled, very broad battlefield terrors with instinctive crude Teknomancy, shamans, necromancers, sentinels, stone cairns, defensive fortifications, and jealousy of Gnome Mekgineer creations; females 10'0"-10'6", males 10'4"-11'0" | First-pass `SK_OGR_Base_A01`, `SK_OGR_Teknomancer_A01`, `SK_OGR_Warrior_Rival_A01`, and `SM_OGR_CairnBattleGate_A01` review imports exist; `SK_OGR_Shaman_A01` and `SK_OGR_Necromancer_A01` packages are ready; needs final sculpt/retopo/UVs/textures and expanded cairn fortification kit |
...
| Manticore | Lion body, bat/draconic wings, scorpion tail, dangerous ruin/desert predator | Base `SK_CRE_Manticore_A01` production package and skeleton/proportion plan are ready for approval; `SK_CRE_Manticore_Interrupt_A01` remains a variant waiting on base approval/import |
...
10. Continue Ogre final-art planning from the imported `SK_OGR_Base_A01`, `SK_OGR_Teknomancer_A01`, and `SK_OGR_Warrior_Rival_A01` review slices, then approve either `SK_OGR_Shaman_A01` or `SK_OGR_Necromancer_A01` for DCC class-fit build.
