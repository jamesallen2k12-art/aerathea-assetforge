...
| `KIT_GNM_OGR_RivalryEncounter_A01` | Source intake and production package ready for 21 Gnome-vs-Ogre encounter images plus 4 added heavy Mek variants; routes shield-wall, Ogre Teknomancer, Ogre warrior, cairn gate, and Manticore interrupt children | Approve first encounter child, recommended `BP_GNM_HeavyMekShieldwall_A01` or `SK_OGR_Teknomancer_A01` |
...
| `SM_INF_CullingTrialFloor_A01` | Blender source and FBX generated, imported to Unreal, material instances assigned, LOD0-LOD3 generated, static mesh sockets added; local startup placement behind the Infernal lineup, offscreen review capture, and validation pass completed | Polish final sculpted slab edges, authored UVs/textures, VFX state Blueprint, and collision fit after cult kit layout is approved |
