...
| `KIT_GNM_OGR_RivalryEncounter_A01` | Source intake and production package ready for 21 Gnome-vs-Ogre encounter images plus 4 added heavy Mek variants; first-pass Unreal implementation now includes shieldwall Blueprint/VFX contract polish, Ogre Teknomancer, Heavy Mek, Ogre Warrior, Ogre cairn gate, crude Tek pylon, base/interrupt Manticore, Ogre Shaman, Ogre Necromancer, pylon and Manticore wrapper Blueprints, and wired native encounter coordinator | Review the updated startup capture, then author final branch VFX/ability timing after approval |
| `VFX_GNM_AetherShieldWall_A01` | First-pass helper mesh/material-state implementation, native shieldwall VFX parameter contract, native Niagara component hook, and empty `NS_GNM_AetherShieldWall_A01` Unreal target exist; final emitter art pass still pending | Author the final Niagara emitters and validate them against the existing native `User.*` parameter contract |
...
| `BP_OGR_CrudeTekPylon_A01` | Native-backed pylon state wrapper created, compiled, placed in startup, and wired to the Gnome/Ogre encounter coordinator with Idle/Charged/Overload/Damaged/Destroyed states | Author final pylon VFX and gameplay repair/destroy rules after review approval |
| `BP_CRE_ManticoreInterrupt_A01` | Native-backed Manticore interrupt wrapper created, compiled, placed in startup, and wired to the encounter coordinator with Hidden/Stalking/Telegraph/LeapImpact/ThreatHold/Retreat states | Author impact Niagara, animation timing, and gameplay traces after review approval |
| `BP_GNM_OGR_BattlefieldEncounter_A01` | Native-backed encounter coordinator implemented; Blueprint asset created, compiled, placed in startup, assigned to required and branch actors, and validated with shieldwall impact, pylon, caster, and Manticore event hooks | Review phase sequencing in the startup capture, then add gameplay timeline/PIE automation |
