...
| `VFX_GNM_AetherShieldWall_A01` | First-pass helper mesh/material-state implementation and native shieldwall VFX parameter contract exist; final Niagara art-pass handoff and build/import status ready; `NS_GNM_AetherShieldWall_A01` not authored | Approve Blueprint-only or native-backed Niagara implementation before authoring the Unreal Niagara asset |
...
| `BP_GNM_OGR_BattlefieldEncounter_A01` | Production package, implementation handoff, build/import status, dependency contract, phase-state plan, actor slots, collision volumes, variables, events, and optional-branch gates ready | Review coordinator contract before Blueprint/native implementation |
