# SM_MKG_ShrinkRay_A01 Production Package

## Art Direction Summary

- Asset name: `SM_MKG_ShrinkRay_A01`
- Asset type: Static Mesh device/tool weapon
- Source: `Gnome Armory.png#Utility_ShrinkRay`
...
Supports signature experimental device with future VFX socket, kept as a readable prop first. It can also serve future loot, crafting, vendor preview, armory display, equipment socket, and startup-scene review workflows where appropriate.

## Silhouette Notes

Primary read: short ray-device barrel, blue lens, brass ring frame, stockless grip, and sturdy tripod-like lower brace. Preserve the large readable forms first; any tiny screws, rivets, wire runs, gauge ticks, and surface scratches must be texture or normal-map detail.

## Scale Notes

45-70 cm long; muzzle socket named `socket_beam`. Author in centimeters. Keep handles, grips, buttons, and attachment zones usable by compact 3-4 ft gnomes rather than human-scale proportions.
...
Create an original stylized fantasy MMORPG concept image of `SM_MKG_ShrinkRay_A01`, a static mesh device/tool weapon for the world of Aerathea. The design should emphasize short ray-device barrel, blue lens, brass ring frame, stockless grip, and sturdy tripod-like lower brace, compact gnome-scale ergonomics, brass and dark-iron engineering, dark leather utility details, restrained blue Aetherium accents, curious Mekgineer culture, and signature experimental device with future VFX socket, kept as a readable prop first. Use hand-painted texture detail, readable shapes, baked-AO-style depth, normal-map-style surface detail, sparing emissive accents, and MMO-friendly production design. Present it as a production asset sheet with front, side, three-quarter view, scale callout, and material callouts on a clean background. Avoid copying any existing franchise and avoid excessive micro-detail that would not translate to a mid-poly Unreal asset.
...
- `T_MKG_ShrinkRay_A01_BC`
- `T_MKG_ShrinkRay_A01_N`
- `T_MKG_ShrinkRay_A01_ORM`
- `T_MKG_ShrinkRay_A01_E` only for the explicit blue Aetherium lens/core/window
- Prefer one material slot; use two only when a backpack, rifle, shield, or configuration needs a shared core material split.

## Triangle Budget

LOD0 2.5k-5k tris, 1-2 material slots.
...
Equipped collision disabled. World display uses one simple box pickup/display bound. Fire traces/projectiles are gameplay-owned and should originate from the documented muzzle socket.

## Animation Notes

Static mesh baseline. Future VFX starts from the documented effect socket and is owned by a Blueprint or ability system.

## Unreal Import Notes

- Unreal path: `/Game/Aerathea/Weapons/Mekgineer/SM_MKG_ShrinkRay_A01`
...
- Docs: `docs/assets/props/SM_MKG_ShrinkRay_A01/`
- Source: `SourceAssets/Blender/Kits/Mekgineer/Armory/SM_MKG_ShrinkRay_A01/`
- Export: `SourceAssets/Exports/Kits/Mekgineer/Armory/SM_MKG_ShrinkRay_A01/`
- Unreal: `/Game/Aerathea/Weapons/Mekgineer/SM_MKG_ShrinkRay_A01`
