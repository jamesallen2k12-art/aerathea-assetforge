...
| `Saved/AssetForgeResearch/SiegeBreaker/A12_R8_SixViewGeneration/InternalRejected_A01/` | `quarantined; local only` | Six generated orthographic sheets plus two front-ratio correction attempts; visually opened but invalid for pixel math because actual ratios miss requirements by `9.66%-31.96%`. |
| `manifests/A12_R8_SIX_VIEW_ORTHOGRAPHIC_GENERATION_INTERNAL_REJECTIONS.md` | `authoritative rejection/status record` | Hashes the local outputs, records exact prompt measurements, deterministic scan failures, and the dimension-locked physical-master stop gate. |
| `Saved/AssetForgeResearch/SiegeBreaker/A12_R8_SixViewGeneration/VisualReference_A01/` | `reference only; local saved set` | Exact hash-matched six images preserved after Flamestrike's positive `looks great` feedback; not visual canon or pixel authority. |
| `handoffs/A12_R8_SIX_VIEW_VISUAL_REFERENCE_RESET_HANDOFF.md` | `authoritative reset-safe handoff` | Detailed state, image hashes, measurement boundary, read order, prohibited actions, and next dimension-locked-master approval gate. |
