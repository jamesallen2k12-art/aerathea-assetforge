...
- Final Niagara art-pass status: handoff ready; emitter art pass pending.
- Unreal Niagara status: empty target system `NS_GNM_AetherShieldWall_A01` exists for native/component binding.
- Native Niagara hook status: implemented in `AAETHeavyMekShieldwallActor`; helper panels remain the visible fallback until the target system receives authored emitters.
...
## Planned Final Niagara Emitters
...
- Startup validation checks the current material/scalar parameter contract and the `NS_GNM_AetherShieldWall_A01` target asset.

## Blocking Items

- Final Niagara emitters are not authored in Unreal.
- Final VFX visual approval must happen after the target system contains authored emitters and is captured from the review camera.

## Remaining To Finalize

1. Author emitters inside `NS_GNM_AetherShieldWall_A01` in Unreal.
