...
- Intake status: visual classification complete; first child approved by Flamestrike on 2026-06-28 as `SK_GNM_IonaSiegebreakerMek_A01`; Mek first-pass DCC/Unreal review implementation imported and validated
...
| --- | --- | --- | --- | --- |
| `Iona.png#Pilot_Iona` | Named gnome hero | `SK_GNM_IonaPilot_A01` | Package needed after Mek scale/cockpit lock | Compact gnome pilot with goggles, bright hair, expressive face, gloves, harness |
| `Iona.png#Mek_Siegebreaker` | Heavy Mek skeletal mesh | `SK_GNM_IonaSiegebreakerMek_A01` | Approved first child; first-pass DCC/Unreal review implementation validated | Large dark-iron/brass Mek frame with fists, boots, cockpit/harness, core |
| `Iona.png#Weapon_TwinArcCannons` | Weapon assembly | `SM_GNM_IonaArcCannon_A01` | Package needed after Mek cannon socket lock | Twin back/shoulder arc cannon assemblies with blue Aetherium muzzle glow |
| `Iona.png#Blueprint_AssembledHero` | Blueprint actor | `BP_GNM_IonaSiegebreaker_A01` | Package needed after pilot/Mek/cannon approval | Assembled pilot, Mek, weapon sockets, VFX, and animation state routing |
...
`SK_GNM_IonaSiegebreakerMek_A01` is now the validated first-pass hero Mek foundation. Split `SK_GNM_IonaPilot_A01` and `SM_GNM_IonaArcCannon_A01` after the Mek scale, cockpit, and cannon socket contract is accepted. Do not treat the first-pass review mesh as final art.
