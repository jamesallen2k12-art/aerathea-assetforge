...
HEAVY_MEK_TAG = "AET_GNOME_HEAVY_MEK_REVIEW"
FBX_IMPORT_UNIFORM_SCALE = 0.01


HEAVY_MEK_ASSET = {
    "name": "SK_GNM_HeavyMek_Rivalry_A01",
    "source": ROOT / "SourceAssets/Exports/Characters/Gnomes/Meks/SK_GNM_HeavyMek_Rivalry_A01/SK_GNM_HeavyMek_Rivalry_A01.fbx",
    "destination": "/Game/Aerathea/Characters/Gnomes/Meks",
    "physics_asset": "/Game/Aerathea/Characters/Gnomes/Meks/PHYS_GNM_HeavyMek_Rivalry_A01",
    "anim_blueprint": "/Game/Aerathea/Characters/Gnomes/Meks/ABP_GNM_HeavyMek_Rivalry_A01",
    "skeleton": "/Game/Aerathea/Characters/Gnomes/Meks/SK_GNM_HeavyMek_Rivalry_A01_Skeleton",
    "actor_label": "AET_PROD_GNM_HeavyMek_Rivalry_A01",
    "actor_location": unreal.Vector(520.0, 320.0, 0.0),
    "actor_yaw": -36.0,
}


HEAVY_MEK_SOCKET_SPECS = [
    ("vfx_reactor_core", "reactor", unreal.Vector(-28.0, 0.0, 54.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("vfx_shield_l", "lowerarm_l", unreal.Vector(28.0, 22.0, 24.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("vfx_shield_r", "upperarm_r", unreal.Vector(22.0, -16.0, 18.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("weapon_cannon_muzzle", "cannon_l", unreal.Vector(72.0, 0.0, -2.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("pilot_hatch", "cockpit", unreal.Vector(20.0, 0.0, 34.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("foot_l", "foot_l", unreal.Vector(28.0, 0.0, -4.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("foot_r", "foot_r", unreal.Vector(28.0, 0.0, -4.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("vfx_stomp_l", "foot_l", unreal.Vector(36.0, 0.0, -6.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("vfx_stomp_r", "foot_r", unreal.Vector(36.0, 0.0, -6.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("weapon_hammer_socket", "hammer_r", unreal.Vector(72.0, 0.0, -4.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("vfx_chest_core", "torso", unreal.Vector(78.0, 0.0, 62.0), unreal.Rotator(0.0, 0.0, 0.0)),
]
...
def ensure_materials():
    return {
        "M_GNM_Mek_DarkIron_Blockout_A01": color_material(
            "M_GNM_Mek_DarkIron_Blockout_A01",
            unreal.LinearColor(0.08, 0.09, 0.10, 1.0),
            roughness=0.68,
            metallic=0.65,
        ),
        "M_GNM_Mek_Brass_Blockout_A01": color_material(
            "M_GNM_Mek_Brass_Blockout_A01",
            unreal.LinearColor(0.78, 0.55, 0.25, 1.0),
            roughness=0.52,
            metallic=0.78,
        ),
        "M_GNM_Mek_Copper_Blockout_A01": color_material(
            "M_GNM_Mek_Copper_Blockout_A01",
            unreal.LinearColor(0.52, 0.25, 0.10, 1.0),
            roughness=0.58,
            metallic=0.72,
        ),
        "M_GNM_Mek_BluePanel_Blockout_A01": color_material(
            "M_GNM_Mek_BluePanel_Blockout_A01",
            unreal.LinearColor(0.05, 0.16, 0.34, 1.0),
            roughness=0.74,
            metallic=0.35,
        ),
        "M_GNM_Mek_AetheriumGlow_Blockout_A01": color_material(
            "M_GNM_Mek_AetheriumGlow_Blockout_A01",
            unreal.LinearColor(0.0, 0.55, 1.0, 1.0),
            roughness=0.24,
            emissive=unreal.LinearColor(0.0, 1.2, 3.8, 1.0),
        ),
        "M_GNM_MekPilotSkin_Blockout_A01": color_material(
            "M_GNM_MekPilotSkin_Blockout_A01",
            unreal.LinearColor(0.78, 0.55, 0.38, 1.0),
        ),
        "M_GNM_MekPilotWorkwear_Blockout_A01": color_material(
            "M_GNM_MekPilotWorkwear_Blockout_A01",
            unreal.LinearColor(0.18, 0.24, 0.29, 1.0),
        ),
        "M_GNM_MekPilotHair_Blockout_A01": color_material(
            "M_GNM_MekPilotHair_Blockout_A01",
            unreal.LinearColor(0.38, 0.12, 0.62, 1.0),
        ),
        "M_GNM_Mek_Leather_Blockout_A01": color_material(
            "M_GNM_Mek_Leather_Blockout_A01",
            unreal.LinearColor(0.18, 0.10, 0.06, 1.0),
        ),
    }
...
def material_instance_name(mesh_name, material_name):
    ...
    if semantic.startswith("M_GNM_"):
        semantic = semantic[len("M_GNM_"):]
    if semantic.endswith("_Blockout_A01"):
        semantic = semantic[: -len("_Blockout_A01")]
...
def assign_project_materials(mesh, project_materials):
    ...
    unreal.log("Synced {} Heavy Mek material slots for {}".format(len(slots), mesh.get_name()))
...
def ensure_skeletal_mesh_sockets(mesh):
    for socket_name, _bone_name, _location, _rotation in HEAVY_MEK_SOCKET_SPECS:
        remove_socket_name(mesh, socket_name)
        remove_socket_name(mesh, "SOCKET_{}".format(socket_name))

    for socket_name, bone_name, location, rotation in HEAVY_MEK_SOCKET_SPECS:
...
def tag_actor(actor):
    ...
    for tag in (unreal.Name(HEAVY_MEK_TAG), unreal.Name("AET_NEXT_SLICE")):
        if tag not in tags:
            tags.append(tag)
...
def update_startup_level(mesh):
    ...
    spawn_skeletal_mesh(HEAVY_MEK_ASSET, mesh)
    if not unreal.EditorLevelLibrary.save_current_level():
        raise RuntimeError("Failed to save current level")
    for directory in (
        HEAVY_MEK_ASSET["destination"],
        MATERIAL_PATH,
...
def main():
    ...
    mesh = import_skeletal_mesh(HEAVY_MEK_ASSET)
    skeleton = skeletal_mesh_skeleton(mesh)
    if skeleton is not None:
        unreal.log("{} skeleton: {}".format(mesh.get_name(), asset_path_without_object(skeleton)))
    assign_project_materials(mesh, materials)
    ensure_physics_asset(mesh, HEAVY_MEK_ASSET["physics_asset"])
    ensure_review_lods_skeletal(mesh)
    ensure_skeletal_mesh_sockets(mesh)
    ensure_anim_blueprint(
        HEAVY_MEK_ASSET["anim_blueprint"],
        HEAVY_MEK_ASSET["skeleton"],
        "{}/{}".format(HEAVY_MEK_ASSET["destination"], HEAVY_MEK_ASSET["name"]),
    )
    update_startup_level(mesh)
    unreal.log("Aerathea Gnome Heavy Mek import complete.")
