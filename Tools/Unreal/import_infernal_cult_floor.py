...
SOURCE = ROOT / "SourceAssets/Exports/Props/Infernals/BalgorothCult/SM_INF_CullingTrialFloor_A01/SM_INF_CullingTrialFloor_A01.fbx"
...
def ensure_materials():
    return {
        "M_INF_CultStone_Blockout_A01": color_material(
            "M_INF_CultStone_Blockout_A01",
            unreal.LinearColor(0.055, 0.060, 0.065, 1.0),
        ),
        "M_INF_ScorchedStone_Blockout_A01": color_material(
            "M_INF_ScorchedStone_Blockout_A01",
            unreal.LinearColor(0.24, 0.060, 0.035, 1.0),
        ),
        "M_INF_ObsidianIron_Blockout_A01": color_material(
            "M_INF_ObsidianIron_Blockout_A01",
            unreal.LinearColor(0.018, 0.018, 0.022, 1.0),
            roughness=0.62,
            metallic=0.45,
        ),
        "M_INF_RitualGlow_Blockout_A01": color_material(
            "M_INF_RitualGlow_Blockout_A01",
            unreal.LinearColor(1.0, 0.18, 0.025, 1.0),
            roughness=0.25,
            emissive=unreal.LinearColor(3.0, 0.22, 0.025, 1.0),
        ),
    }
...
def material_instance_name(mesh_name, material_name):
    ...
    if semantic.startswith("M_INF_"):
        semantic = semantic[len("M_INF_") :]
...
def import_static_mesh():
    ...
    unreal.EditorAssetLibrary.save_asset(asset_path)
    unreal.log("Imported Infernal cult floor static mesh: {}".format(asset_path))
    return mesh
...
def ensure_review_lods_static(mesh, target_lods=4):
    ...
    settings = []
    for percent, screen_size in ((1.0, 1.0), (0.60, 0.55), (0.32, 0.28), (0.16, 0.12)):
...
def ensure_static_mesh_sockets(mesh):
    socket_specs = [
        ("vfx_center", unreal.Vector(0.0, 0.0, 28.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("vfx_ring_active", unreal.Vector(0.0, 305.0, 23.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("vfx_rejected_gap", unreal.Vector(330.0, 0.0, 23.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("snap_altar", unreal.Vector(0.0, -185.0, 24.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("snap_arch_front", unreal.Vector(0.0, -455.0, 0.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("stage_spawn", unreal.Vector(-250.0, -125.0, 20.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("stage_blooded", unreal.Vector(0.0, -165.0, 20.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("stage_elder", unreal.Vector(250.0, -125.0, 20.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ]
...
def spawn_floor_actor(mesh):
    label = "AET_PROD_INF_CullingTrialFloor_A01"
    ...
    if actor is None or actor.get_class().get_name() != "StaticMeshActor":
        ...
        actor = unreal.EditorLevelLibrary.spawn_actor_from_class(
            ...
            unreal.Vector(0.0, 110.0, -20.0),
            unreal.Rotator(0.0, 0.0, 0.0),
        )
    if actor is None:
        raise RuntimeError("Failed to spawn {}".format(label))
    actor.set_actor_label(label)
    actor.set_actor_location(unreal.Vector(0.0, 110.0, -20.0), False, True)
...
def update_startup_level(mesh):
    ...
    unreal.log("Updated startup level with Infernal cult floor.")
...
def main():
    ...
    unreal.log("Aerathea Infernal cult floor import complete.")
