...
MESH_PATH = "/Game/Aerathea/Props/Giants/BloodAxe/Cairns/SM_GIA_BloodAxeCairnSlabCluster_A01_FinalTrial"
TEXTURE_BASE = "/Game/Aerathea/Textures/Giants/BloodAxe/Cairns/SM_GIA_BloodAxeCairnSlabCluster_A01_FinalTrial"
TEXTURE_LABELS = ["A1Projection", "Stone", "DarkStone", "EarthAsh", "Rawhide", "BloodAxeRedPaint"]
TEXTURE_PATHS = [
    "{}/T_GIA_BloodAxeCairnSlabCluster_A01_FinalTrial_{}_{}".format(TEXTURE_BASE, label, suffix)
    for label in TEXTURE_LABELS
    for suffix in ("BC", "N", "ORM")
]
MATERIAL_PATHS = [
    "/Game/Aerathea/Materials/Giants/BloodAxe/Cairns/M_GIA_BloodAxeCairn_FinalTrial_A1Projection_A01",
    "/Game/Aerathea/Materials/Giants/BloodAxe/Cairns/M_GIA_BloodAxeCairn_FinalTrial_Stone_A01",
    "/Game/Aerathea/Materials/Giants/BloodAxe/Cairns/M_GIA_BloodAxeCairn_FinalTrial_DarkStone_A01",
    "/Game/Aerathea/Materials/Giants/BloodAxe/Cairns/M_GIA_BloodAxeCairn_FinalTrial_EarthAsh_A01",
    "/Game/Aerathea/Materials/Giants/BloodAxe/Cairns/M_GIA_BloodAxeCairn_FinalTrial_Rawhide_A01",
    "/Game/Aerathea/Materials/Giants/BloodAxe/Cairns/M_GIA_BloodAxeCairn_FinalTrial_BloodAxeRedPaint_A01",
]
MATERIAL_INSTANCE_PATHS = [
    "/Game/Aerathea/Materials/Instances/MI_GIA_BloodAxeCairnSlabCluster_A01_FinalTrial_A1Projection",
    "/Game/Aerathea/Materials/Instances/MI_GIA_BloodAxeCairnSlabCluster_A01_FinalTrial_Stone",
    "/Game/Aerathea/Materials/Instances/MI_GIA_BloodAxeCairnSlabCluster_A01_FinalTrial_DarkStone",
    "/Game/Aerathea/Materials/Instances/MI_GIA_BloodAxeCairnSlabCluster_A01_FinalTrial_EarthAsh",
    "/Game/Aerathea/Materials/Instances/MI_GIA_BloodAxeCairnSlabCluster_A01_FinalTrial_Rawhide",
    "/Game/Aerathea/Materials/Instances/MI_GIA_BloodAxeCairnSlabCluster_A01_FinalTrial_BloodAxeRedPaint",
]

ACTOR_LABEL = "AET_PROD_GIA_BloodAxeCairnSlabCluster_A01_FinalTrial"
EXPECTED_LOCATION = unreal.Vector(12480.0, 10000.0, 0.0)
EXPECTED_YAW = -18.0
EXPECTED_TAGS = [
    "AET_FIRST_SLICE",
    "AET_BLOODAXE_CAIRN_FINAL_TRIAL_REVIEW",
    "AET_STATIC_REVIEW_TARGET",
]

COMMON_METADATA = {
    "Aerathea.StaticMesh.Package": "SM_GIA_BloodAxeCairnSlabCluster_A01_FinalTrial",
...
def validate_mesh_material_slots(failures, mesh):
    ...
    if len(slots) < 6:
        failures.append("{} has {} material slots, expected at least 6".format(MESH_PATH, len(slots)))
    assigned_paths = set()
    for slot in slots:
...
def validate_mesh_bounds(failures, mesh, measurements):
    ...
    if height < 130.0 or height > 190.0:
        failures.append("{} height {:.2f} cm outside expected final-trial cairn envelope".format(MESH_PATH, height))
    if width < 300.0 or width > 420.0:
        failures.append("{} width {:.2f} cm outside expected final-trial cairn envelope".format(MESH_PATH, width))
    if depth < 190.0 or depth > 290.0:
        failures.append("{} depth {:.2f} cm outside expected final-trial cairn envelope".format(MESH_PATH, depth))
...
def main():
    ...
    if failures:
        ...
        raise RuntimeError("Blood Axe final-trial cairn validation failed with {} issue(s)".format(len(failures)))

    height, width, depth = measurements[0] if measurements else ("unavailable", "unavailable", "unavailable")
    unreal.log(
        "Blood Axe final-trial cairn validation passed: {:.2f}h x {:.2f}w x {:.2f}d cm, 4 LODs, 6 materials, 18 textures, startup actor {}, broad collision enabled.".format(
            height,
            width,
            depth,
            ACTOR_LABEL,
