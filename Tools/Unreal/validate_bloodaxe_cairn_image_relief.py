...
TEXTURE_PATHS = [
    "/Game/Aerathea/Textures/Giants/BloodAxe/Cairns/SM_GIA_BloodAxeCairnSlabCluster_A01_ImageRelief/T_GIA_BloodAxeCairnSlabCluster_A01_ImageRelief_BC",
    "/Game/Aerathea/Textures/Giants/BloodAxe/Cairns/SM_GIA_BloodAxeCairnSlabCluster_A01_ImageRelief/T_GIA_BloodAxeCairnSlabCluster_A01_ImageRelief_N",
    "/Game/Aerathea/Textures/Giants/BloodAxe/Cairns/SM_GIA_BloodAxeCairnSlabCluster_A01_ImageRelief/T_GIA_BloodAxeCairnSlabCluster_A01_ImageRelief_ORM",
]
PROJECTION_MATERIAL_PATH = "/Game/Aerathea/Materials/Giants/BloodAxe/Cairns/M_GIA_BloodAxeCairn_ImageReliefProjection_A01"
SIDE_MATERIAL_PATH = "/Game/Aerathea/Materials/Giants/BloodAxe/Cairns/M_GIA_BloodAxeCairn_ImageReliefSideStone_A01"
...
EXPECTED_LOCATION = unreal.Vector(10000.0, 10000.0, 0.0)
EXPECTED_YAW = -18.0
EXPECTED_TAGS = [
    "AET_FIRST_SLICE",
...
COMMON_METADATA = {
    "Aerathea.StaticMesh.Package": "SM_GIA_BloodAxeCairnSlabCluster_A01_ImageRelief",
...
def validate_mesh_bounds(failures, mesh, measurements):
    ...
    if height < 190.0 or height > 230.0:
        failures.append("{} height {:.2f} cm outside expected image-relief cairn envelope".format(MESH_PATH, height))
    if max(width, depth) < 300.0 or max(width, depth) > 340.0:
        failures.append("{} max horizontal bounds {:.2f} cm outside expected image-relief cairn envelope".format(MESH_PATH, max(width, depth)))
    if min(width, depth) < 95.0 or min(width, depth) > 145.0:
        failures.append("{} min horizontal bounds {:.2f} cm outside expected image-relief cairn envelope".format(MESH_PATH, min(width, depth)))
...
def main():
    ...
    if failures:
        raise RuntimeError("Blood Axe image-relief cairn validation failed: {}".format("; ".join(failures)))

    if measurements and measurements[0][0] != "unavailable":
        height, width, depth = measurements[0]
        print(
            "Blood Axe image-relief cairn validation passed: {:.2f}h x {:.2f}w x {:.2f}d cm, 4 LODs, 2 materials, 3 textures, startup actor {}, broad collision enabled.".format(
                height,
                width,
                depth,
                ACTOR_LABEL,
        ...
        print(
            "Blood Axe image-relief cairn validation passed: 4 LODs, 2 materials, 3 textures, startup actor {}, broad collision enabled.".format(
                ACTOR_LABEL,
            )
        )
