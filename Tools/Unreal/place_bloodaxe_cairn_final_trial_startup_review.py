...
ACTOR_LOCATION = unreal.Vector(12480.0, 10000.0, 0.0)
ACTOR_YAW = -18.0
REVIEW_TAGS = [
    "AET_FIRST_SLICE",
...
def main():
    ...
    if mesh is None:
        raise RuntimeError("Missing Blood Axe final-trial cairn mesh: {}".format(MESH_PATH))

    materials = []
    for path in MATERIAL_INSTANCE_PATHS:
        material = unreal.load_asset(path)
        if material is None:
            raise RuntimeError("Missing final-trial cairn material instance: {}".format(path))
    ...
    if not unreal.EditorLevelLibrary.save_current_level():
        raise RuntimeError("Failed to save startup level after Blood Axe final-trial cairn placement")
