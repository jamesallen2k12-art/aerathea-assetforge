...
MATERIAL_PATH = "/Game/Aerathea/Materials/Infernals/Rogue"
MATERIAL_INSTANCE_PATH = "/Game/Aerathea/Materials/Instances"
ROGUE_TAG = "AET_INFERNAL_ROGUE_REVIEW"
FBX_IMPORT_UNIFORM_SCALE = 0.01
BASE_SKELETON_PATH = "/Game/Aerathea/Characters/Infernals/Base/SK_INF_Base_Compact_A01_Skeleton"


ROGUE_ASSET = {
    "name": "SK_INF_Rogue_A01",
    "source": ROOT / "SourceAssets/Exports/Characters/Infernals/Rogue/SK_INF_Rogue_A01/SK_INF_Rogue_A01.fbx",
    "destination": "/Game/Aerathea/Characters/Infernals/Rogue",
    "physics_asset": "/Game/Aerathea/Characters/Infernals/Rogue/PHYS_INF_Rogue_A01",
    "anim_blueprint": "/Game/Aerathea/Characters/Infernals/Rogue/ABP_INF_Rogue_A01",
    "skeleton": BASE_SKELETON_PATH,
    "actor_label": "AET_PROD_INF_Rogue_A01",
    "actor_location": unreal.Vector(320.0, 535.0, 0.0),
    "actor_yaw": -42.0,
}
...
ROGUE_SOCKET_SPECS = [
    ...
    ("vfx_ambush_mark", "chest", unreal.Vector(11.0, 0.0, 4.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("vfx_invisible_sight_focus", "head", unreal.Vector(8.0, 0.0, 8.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("pounce_trace", "chest", unreal.Vector(22.0, 0.0, -28.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("claw_rake_trace", "chest", unreal.Vector(24.0, 0.0, -46.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("tail_balance_trace", "tail_02", unreal.Vector(-44.0, 0.0, -10.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("crouch_center", "pelvis", unreal.Vector(8.0, 0.0, -3.0), unreal.Rotator(0.0, 0.0, 0.0)),
]
...
def ensure_materials():
    return {
        "M_INF_Rogue_Skin_Blockout_A01": color_material(
            "M_INF_Rogue_Skin_Blockout_A01",
            unreal.LinearColor(0.52, 0.075, 0.060, 1.0),
        ),
        "M_INF_Rogue_HornClaw_Blockout_A01": color_material(
            "M_INF_Rogue_HornClaw_Blockout_A01",
            unreal.LinearColor(0.025, 0.020, 0.018, 1.0),
            roughness=0.70,
        ),
        "M_INF_Rogue_Wing_Blockout_A01": color_material(
            "M_INF_Rogue_Wing_Blockout_A01",
            unreal.LinearColor(0.10, 0.035, 0.040, 1.0),
        ),
        "M_INF_Rogue_Wraps_Blockout_A01": color_material(
            "M_INF_Rogue_Wraps_Blockout_A01",
            unreal.LinearColor(0.035, 0.034, 0.037, 1.0),
        ),
        "M_INF_Rogue_LightArmor_Blockout_A01": color_material(
            "M_INF_Rogue_LightArmor_Blockout_A01",
            unreal.LinearColor(0.028, 0.026, 0.032, 1.0),
            roughness=0.62,
            metallic=0.45,
        ),
        "M_INF_Rogue_BoneHorn_Blockout_A01": color_material(
            "M_INF_Rogue_BoneHorn_Blockout_A01",
            unreal.LinearColor(0.50, 0.43, 0.32, 1.0),
        ),
        "M_INF_Rogue_SightGlow_Blockout_A01": color_material(
            "M_INF_Rogue_SightGlow_Blockout_A01",
...
def material_instance_name(mesh_name, material_name):
    ...
    for prefix in ("M_INF_Rogue_", "M_INF_"):
...
def assign_project_materials(mesh, project_materials):
    ...
    unreal.log("Synced {} Infernal Rogue material slots for {}".format(len(slots), mesh.get_name()))
...
def import_skeletal_mesh(entry):
    ...
    unreal.EditorAssetLibrary.save_asset(asset_path)
    unreal.log("Imported Infernal Rogue skeletal mesh: {}".format(asset_path))
    return mesh
...
def ensure_skeletal_mesh_sockets(mesh):
    for socket_name, _bone_name, _location, _rotation in ROGUE_SOCKET_SPECS:
        remove_socket_name(mesh, socket_name)
        remove_socket_name(mesh, "SOCKET_{}".format(socket_name))

    for socket_name, bone_name, location, rotation in ROGUE_SOCKET_SPECS:
    ...
    unreal.EditorAssetLibrary.save_loaded_asset(mesh)
    unreal.log("Ensured {} Infernal Rogue sockets on {}".format(len(ROGUE_SOCKET_SPECS), mesh.get_name()))
...
def tag_actor(actor):
    ...
    for tag in (unreal.Name(ROGUE_TAG), unreal.Name("AET_NEXT_SLICE")):
        if tag not in tags:
            tags.append(tag)
...
def update_startup_level(mesh):
    ...
    spawn_skeletal_mesh(
        ROGUE_ASSET["actor_label"],
        mesh,
        ROGUE_ASSET["actor_location"],
        ROGUE_ASSET["actor_yaw"],
    )
    ...
    unreal.log("Updated startup level with Infernal Rogue review actor.")


def main():
    materials = ensure_materials()
    mesh = import_skeletal_mesh(ROGUE_ASSET)
    assign_project_materials(mesh, materials)
    ensure_physics_asset(mesh, ROGUE_ASSET["physics_asset"])
    ensure_review_lods_skeletal(mesh)
    ensure_skeletal_mesh_sockets(mesh)
    ensure_anim_blueprint(
        ROGUE_ASSET["anim_blueprint"],
        ROGUE_ASSET["skeleton"],
        "{}/{}".format(ROGUE_ASSET["destination"], ROGUE_ASSET["name"]),
    )
    update_startup_level(mesh)
    unreal.log("Aerathea Infernal Rogue import complete.")
