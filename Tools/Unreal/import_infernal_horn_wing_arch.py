...
MATERIAL_PATH = "/Game/Aerathea/Materials/Infernals/HornWingArch"
...
SOURCE = ROOT / "SourceAssets/Exports/Props/Infernals/BalgorothCult/SM_INF_HornWingArch_A01/SM_INF_HornWingArch_A01.fbx"
...
def ensure_materials():
    return {
        "M_INF_HornWingArch_CultStone_Blockout_A01": color_material(
            "M_INF_HornWingArch_CultStone_Blockout_A01",
            unreal.LinearColor(0.050, 0.052, 0.057, 1.0),
        ),
        "M_INF_HornWingArch_ScorchedStone_Blockout_A01": color_material(
            "M_INF_HornWingArch_ScorchedStone_Blockout_A01",
            unreal.LinearColor(0.22, 0.055, 0.038, 1.0),
        ),
        "M_INF_HornWingArch_ObsidianIron_Blockout_A01": color_material(
            "M_INF_HornWingArch_ObsidianIron_Blockout_A01",
            unreal.LinearColor(0.016, 0.015, 0.020, 1.0),
            roughness=0.62,
            metallic=0.42,
        ),
        "M_INF_HornWingArch_BoneHorn_Blockout_A01": color_material(
            "M_INF_HornWingArch_BoneHorn_Blockout_A01",
            unreal.LinearColor(0.57, 0.48, 0.34, 1.0),
            roughness=0.70,
        ),
        "M_INF_HornWingArch_BrandGlow_Blockout_A01": color_material(
            "M_INF_HornWingArch_BrandGlow_Blockout_A01",
...
def material_instance_name(mesh_name, material_name):
    ...
    for prefix in ("M_INF_HornWingArch_", "M_INF_"):
...
def assign_project_materials(mesh, project_materials):
    ...
    unreal.log("Synced {} Infernal horn-wing arch material slots for {}".format(len(slots), mesh.get_name()))
...
def import_static_mesh():
    ...
    unreal.EditorAssetLibrary.save_asset(asset_path)
    unreal.log("Imported Infernal horn-wing arch static mesh: {}".format(asset_path))
    return mesh
...
def ensure_review_lods_static(mesh, target_lods=4):
    ...
    settings = []
    for percent, screen_size in ((1.0, 1.0), (0.62, 0.55), (0.34, 0.28), (0.18, 0.12)):
...
def ensure_static_mesh_sockets(mesh):
    socket_specs = [
        ("snap_floor", unreal.Vector(0.0, -112.0, 0.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("snap_altar", unreal.Vector(0.0, -210.0, 0.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("guard_l", unreal.Vector(-250.0, -205.0, 0.0), unreal.Rotator(0.0, 0.0, -12.0)),
        ("guard_r", unreal.Vector(250.0, -205.0, 0.0), unreal.Rotator(0.0, 0.0, 12.0)),
        ("banner_l", unreal.Vector(-222.0, -104.0, 376.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("banner_r", unreal.Vector(222.0, -104.0, 376.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("vfx_crown", unreal.Vector(0.0, -106.0, 612.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("vfx_eye", unreal.Vector(0.0, -116.0, 506.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("vfx_inner_throat", unreal.Vector(0.0, -116.0, 250.0), unreal.Rotator(0.0, 0.0, 0.0)),
        ("vfx_rejected_gap", unreal.Vector(0.0, -118.0, 172.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ]
...
def spawn_arch_actor(mesh):
    label = "AET_PROD_INF_HornWingArch_A01"
    ...
    if actor is None or actor.get_class().get_name() != "StaticMeshActor":
        ...
        actor = unreal.EditorLevelLibrary.spawn_actor_from_class(
            ...
            unreal.Vector(0.0, -345.0, 0.0),
            unreal.Rotator(0.0, 0.0, 0.0),
        )
    if actor is None:
        raise RuntimeError("Failed to spawn {}".format(label))
    actor.set_actor_label(label)
    actor.set_actor_location(unreal.Vector(0.0, -345.0, 0.0), False, True)
...
def update_startup_level(mesh):
    ...
    unreal.log("Updated startup level with Infernal horn-wing arch.")
...
def main():
    ...
    unreal.log("Aerathea Infernal horn-wing arch import complete.")
