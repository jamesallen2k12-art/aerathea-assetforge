...
def main():
    ...
    if mesh is None:
        raise RuntimeError("Missing Blood Axe complete cairn mesh: {}".format(MESH_PATH))

    materials = []
    for path in MATERIAL_INSTANCE_PATHS:
        material = unreal.load_asset(path)
        if material is None:
            raise RuntimeError("Missing complete cairn material instance: {}".format(path))
    ...
    if not unreal.EditorLevelLibrary.save_current_level():
        raise RuntimeError("Failed to save startup level after Blood Axe complete cairn placement")
