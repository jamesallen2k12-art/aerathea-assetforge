...
BLACKPIKE_TAG = "AET_ABY_BLACKPIKE_REVIEW"
FBX_IMPORT_UNIFORM_SCALE = 0.01


BLACKPIKE_ASSET = {
    "name": "SK_ABY_BlackPikeTrooper_A01",
    "source": ROOT / "SourceAssets/Exports/Creatures/Abyss/SK_ABY_BlackPikeTrooper_A01/SK_ABY_BlackPikeTrooper_A01.fbx",
    "destination": "/Game/Aerathea/Creatures/Abyss/Troops/BlackPikeTrooper",
    "physics_asset": "/Game/Aerathea/Creatures/Abyss/Troops/BlackPikeTrooper/PHYS_ABY_BlackPikeTrooper_A01",
    "anim_blueprint": "/Game/Aerathea/Creatures/Abyss/Troops/BlackPikeTrooper/ABP_ABY_Trooper_A01",
    "skeleton": "/Game/Aerathea/Creatures/Abyss/Troops/BlackPikeTrooper/SK_ABY_BlackPikeTrooper_A01_Skeleton",
    "actor_label": "AET_PROD_ABY_BlackPikeTrooper_A01",
    "actor_location": unreal.Vector(1040.0, 220.0, 0.0),
    "actor_yaw": -42.0,
}


BLACKPIKE_SOCKET_SPECS = [
    ("socket_weapon_r", "hand_r", unreal.Vector(8.0, -14.0, 6.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("socket_weapon_l", "hand_l", unreal.Vector(8.0, 14.0, 6.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("socket_pike_tip", "hand_r", unreal.Vector(-14.0, -34.0, 238.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("socket_head_vfx", "head", unreal.Vector(10.0, 0.0, 18.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("socket_eye_l", "head", unreal.Vector(22.0, 7.0, 5.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("socket_eye_r", "head", unreal.Vector(22.0, -7.0, 5.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("socket_chest_core", "chest", unreal.Vector(38.0, 0.0, -6.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("socket_banner_back", "chest", unreal.Vector(-38.0, 0.0, 26.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("socket_ground_rift", "root", unreal.Vector(42.0, 0.0, 3.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("socket_hit_trace_pike", "hand_r", unreal.Vector(-8.0, -30.0, 166.0), unreal.Rotator(0.0, 0.0, 0.0)),
]
...
def ensure_materials():
    return {
        "M_ABY_CharredFlesh_Blockout_A01": color_material(
            "M_ABY_CharredFlesh_Blockout_A01",
            unreal.LinearColor(0.08, 0.065, 0.065, 1.0),
        ),
        "M_ABY_ScorchedIron_Blockout_A01": color_material(
            "M_ABY_ScorchedIron_Blockout_A01",
            unreal.LinearColor(0.055, 0.052, 0.050, 1.0),
            roughness=0.76,
            metallic=0.60,
        ),
        "M_ABY_AshCloth_Blockout_A01": color_material(
            "M_ABY_AshCloth_Blockout_A01",
            unreal.LinearColor(0.075, 0.060, 0.085, 1.0),
        ),
        "M_ABY_BoneHorn_Blockout_A01": color_material(
            "M_ABY_BoneHorn_Blockout_A01",
            unreal.LinearColor(0.58, 0.49, 0.36, 1.0),
        ),
        "M_ABY_PikeBlackIron_Blockout_A01": color_material(
            "M_ABY_PikeBlackIron_Blockout_A01",
            unreal.LinearColor(0.035, 0.034, 0.036, 1.0),
            roughness=0.72,
            metallic=0.65,
        ),
        "M_ABY_VoidGlow_Blockout_A01": color_material(
            "M_ABY_VoidGlow_Blockout_A01",
            unreal.LinearColor(0.55, 0.10, 1.00, 1.0),
            roughness=0.25,
            emissive=unreal.LinearColor(1.25, 0.18, 2.60, 1.0),
        ),
        "M_ABY_EmberFissure_Blockout_A01": color_material(
            "M_ABY_EmberFissure_Blockout_A01",
            unreal.LinearColor(1.00, 0.22, 0.04, 1.0),
            roughness=0.35,
            emissive=unreal.LinearColor(2.40, 0.38, 0.08, 1.0),
        ),
    }
...
def assign_project_materials(mesh, project_materials):
    ...
    unreal.log("Synced {} Abyss material slots for {}".format(len(slots), mesh.get_name()))
...
def import_skeletal_mesh(entry):
    ...
    unreal.EditorAssetLibrary.save_asset(asset_path)
    unreal.log("Imported Abyss BlackPike skeletal mesh: {}".format(asset_path))
    return mesh
...
def ensure_skeletal_mesh_sockets(mesh):
    for socket_name, _bone_name, _location, _rotation in BLACKPIKE_SOCKET_SPECS:
        remove_socket_name(mesh, socket_name)
        remove_socket_name(mesh, "SOCKET_{}".format(socket_name))

    for socket_name, bone_name, location, rotation in BLACKPIKE_SOCKET_SPECS:
    ...
    unreal.EditorAssetLibrary.save_loaded_asset(mesh)
    unreal.log("Ensured {} Abyss BlackPike sockets on {}".format(len(BLACKPIKE_SOCKET_SPECS), mesh.get_name()))
...
def tag_actor(actor):
    ...
    for tag in (unreal.Name(BLACKPIKE_TAG), unreal.Name("AET_NEXT_SLICE")):
        if tag not in tags:
            tags.append(tag)
...
def update_startup_level(mesh):
    ...
    spawn_skeletal_mesh(
        BLACKPIKE_ASSET["actor_label"],
        mesh,
        BLACKPIKE_ASSET["actor_location"],
        BLACKPIKE_ASSET["actor_yaw"],
    )
    ...
    unreal.log("Updated startup level with Abyss BlackPike review actor.")


def main():
    materials = ensure_materials()
    mesh = import_skeletal_mesh(BLACKPIKE_ASSET)
    assign_project_materials(mesh, materials)
    ensure_physics_asset(mesh, BLACKPIKE_ASSET["physics_asset"])
    ensure_review_lods_skeletal(mesh)
    ensure_skeletal_mesh_sockets(mesh)
    ensure_anim_blueprint(
        BLACKPIKE_ASSET["anim_blueprint"],
        BLACKPIKE_ASSET["skeleton"],
        "{}/{}".format(BLACKPIKE_ASSET["destination"], BLACKPIKE_ASSET["name"]),
    )
    update_startup_level(mesh)
    unreal.log("Aerathea Abyss BlackPike Trooper import complete.")
