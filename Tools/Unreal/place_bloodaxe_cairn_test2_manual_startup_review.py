...
ACTOR_LOCATION = unreal.Vector(12880.0, 10000.0, 0.0)
ACTOR_YAW = -18.0
REVIEW_TAGS = [
    "AET_FIRST_SLICE",
...
def main():
    ...
    if mesh is None:
        raise RuntimeError("Missing Blood Axe test2-manual mesh: {}".format(MESH_PATH))
    projection_material = unreal.load_asset(PROJECTION_MI_PATH)
    if projection_material is None:
        raise RuntimeError("Missing Blood Axe test2-manual projection material: {}".format(PROJECTION_MI_PATH))
    side_material = unreal.load_asset(SIDE_MI_PATH)
    if side_material is None:
        raise RuntimeError("Missing Blood Axe test2-manual side material: {}".format(SIDE_MI_PATH))
    ...
    if not unreal.EditorLevelLibrary.save_current_level():
        raise RuntimeError("Failed to save startup level after Blood Axe test2-manual placement")
