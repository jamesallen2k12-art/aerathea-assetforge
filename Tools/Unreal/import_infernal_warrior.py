...
MATERIAL_PATH = "/Game/Aerathea/Materials/Infernals/Warrior"
MATERIAL_INSTANCE_PATH = "/Game/Aerathea/Materials/Instances"
WARRIOR_TAG = "AET_INFERNAL_WARRIOR_REVIEW"
FBX_IMPORT_UNIFORM_SCALE = 0.01
BASE_SKELETON_PATH = "/Game/Aerathea/Characters/Infernals/Base/SK_INF_Base_Tall_A01_Skeleton"


WARRIOR_ASSET = {
    "name": "SK_INF_Warrior_A01",
    "source": ROOT / "SourceAssets/Exports/Characters/Infernals/Warrior/SK_INF_Warrior_A01/SK_INF_Warrior_A01.fbx",
    "destination": "/Game/Aerathea/Characters/Infernals/Warrior",
    "physics_asset": "/Game/Aerathea/Characters/Infernals/Warrior/PHYS_INF_Warrior_A01",
    "anim_blueprint": "/Game/Aerathea/Characters/Infernals/Warrior/ABP_INF_Warrior_A01",
    "skeleton": BASE_SKELETON_PATH,
    "actor_label": "AET_PROD_INF_Warrior_A01",
    "actor_location": unreal.Vector(800.0, 535.0, 0.0),
    "actor_yaw": -42.0,
}


WARRIOR_SOCKET_SPECS = [
    ...
    ("hand_l_cast", "hand_l", unreal.Vector(18.0, 2.0, 8.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("hand_r_cast", "hand_r", unreal.Vector(18.0, -2.0, 8.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("vfx_hand_l", "hand_l", unreal.Vector(18.0, 2.0, 8.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("vfx_hand_r", "hand_r", unreal.Vector(18.0, -2.0, 8.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ...
    ("wing_l_tip", "wing_l", unreal.Vector(-56.0, 24.0, -14.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("wing_r_tip", "wing_r", unreal.Vector(-56.0, -24.0, -14.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("tail_tip", "tail_02", unreal.Vector(-42.0, 0.0, -10.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("vfx_tail_tip", "tail_02", unreal.Vector(-42.0, 0.0, -10.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ...
    ("vfx_rage_core", "chest", unreal.Vector(14.0, 0.0, 6.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("tail_sweep_trace", "tail_02", unreal.Vector(-62.0, 0.0, -8.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("wing_buffet_trace", "chest", unreal.Vector(-28.0, 0.0, 8.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("body_check_trace", "chest", unreal.Vector(18.0, 0.0, -2.0), unreal.Rotator(0.0, 0.0, 0.0)),
]
...
def ensure_materials():
    return {
        "M_INF_Warrior_Skin_Blockout_A01": color_material(
            "M_INF_Warrior_Skin_Blockout_A01",
            unreal.LinearColor(0.62, 0.11, 0.075, 1.0),
        ),
        "M_INF_Warrior_HornClaw_Blockout_A01": color_material(
            "M_INF_Warrior_HornClaw_Blockout_A01",
            unreal.LinearColor(0.025, 0.020, 0.018, 1.0),
            roughness=0.70,
        ),
        "M_INF_Warrior_Wing_Blockout_A01": color_material(
            "M_INF_Warrior_Wing_Blockout_A01",
            unreal.LinearColor(0.14, 0.045, 0.040, 1.0),
        ),
        "M_INF_Warrior_RitualCloth_Blockout_A01": color_material(
            "M_INF_Warrior_RitualCloth_Blockout_A01",
            unreal.LinearColor(0.075, 0.055, 0.052, 1.0),
        ),
        "M_INF_Warrior_ObsidianArmor_Blockout_A01": color_material(
            "M_INF_Warrior_ObsidianArmor_Blockout_A01",
            unreal.LinearColor(0.035, 0.032, 0.038, 1.0),
            roughness=0.62,
            metallic=0.45,
        ),
        "M_INF_Warrior_BoneOrnaments_Blockout_A01": color_material(
            "M_INF_Warrior_BoneOrnaments_Blockout_A01",
            unreal.LinearColor(0.58, 0.49, 0.35, 1.0),
        ),
        "M_INF_Warrior_AbyssalGlow_Blockout_A01": color_material(
            "M_INF_Warrior_AbyssalGlow_Blockout_A01",
            unreal.LinearColor(1.0, 0.24, 0.035, 1.0),
            roughness=0.25,
            emissive=unreal.LinearColor(2.4, 0.35, 0.025, 1.0),
        ),
    }
...
def material_instance_name(mesh_name, material_name):
    ...
    for prefix in ("M_INF_Warrior_", "M_INF_"):
...
def assign_project_materials(mesh, project_materials):
    ...
    unreal.log("Synced {} Infernal Warrior material slots for {}".format(len(slots), mesh.get_name()))
...
def import_skeletal_mesh(entry):
    ...
    unreal.EditorAssetLibrary.save_asset(asset_path)
    unreal.log("Imported Infernal Warrior skeletal mesh: {}".format(asset_path))
    return mesh
...
def ensure_skeletal_mesh_sockets(mesh):
    for socket_name, _bone_name, _location, _rotation in WARRIOR_SOCKET_SPECS:
        remove_socket_name(mesh, socket_name)
        remove_socket_name(mesh, "SOCKET_{}".format(socket_name))

    for socket_name, bone_name, location, rotation in WARRIOR_SOCKET_SPECS:
    ...
    unreal.EditorAssetLibrary.save_loaded_asset(mesh)
    unreal.log("Ensured {} Infernal Warrior sockets on {}".format(len(WARRIOR_SOCKET_SPECS), mesh.get_name()))
...
def tag_actor(actor):
    ...
    for tag in (unreal.Name(WARRIOR_TAG), unreal.Name("AET_NEXT_SLICE")):
        if tag not in tags:
            tags.append(tag)
...
def update_startup_level(mesh):
    ...
    spawn_skeletal_mesh(
        WARRIOR_ASSET["actor_label"],
        mesh,
        WARRIOR_ASSET["actor_location"],
        WARRIOR_ASSET["actor_yaw"],
    )
    ...
    unreal.log("Updated startup level with Infernal Warrior review actor.")


def main():
    materials = ensure_materials()
    mesh = import_skeletal_mesh(WARRIOR_ASSET)
    assign_project_materials(mesh, materials)
    ensure_physics_asset(mesh, WARRIOR_ASSET["physics_asset"])
    ensure_review_lods_skeletal(mesh)
    ensure_skeletal_mesh_sockets(mesh)
    ensure_anim_blueprint(
        WARRIOR_ASSET["anim_blueprint"],
        WARRIOR_ASSET["skeleton"],
        "{}/{}".format(WARRIOR_ASSET["destination"], WARRIOR_ASSET["name"]),
    )
    update_startup_level(mesh)
    unreal.log("Aerathea Infernal Warrior import complete.")
