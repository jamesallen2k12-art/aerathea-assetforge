...
MATERIAL_PATH = "/Game/Aerathea/Materials/Infernals/Mage"
MATERIAL_INSTANCE_PATH = "/Game/Aerathea/Materials/Instances"
MAGE_TAG = "AET_INFERNAL_MAGE_REVIEW"
FBX_IMPORT_UNIFORM_SCALE = 0.01
BASE_SKELETON_PATH = "/Game/Aerathea/Characters/Infernals/Base/SK_INF_Base_Tall_A01_Skeleton"


MAGE_ASSET = {
    "name": "SK_INF_Mage_A01",
    "source": ROOT / "SourceAssets/Exports/Characters/Infernals/Mage/SK_INF_Mage_A01/SK_INF_Mage_A01.fbx",
    "destination": "/Game/Aerathea/Characters/Infernals/Mage",
    "physics_asset": "/Game/Aerathea/Characters/Infernals/Mage/PHYS_INF_Mage_A01",
    "anim_blueprint": "/Game/Aerathea/Characters/Infernals/Mage/ABP_INF_Mage_A01",
    "skeleton": BASE_SKELETON_PATH,
    "actor_label": "AET_PROD_INF_Mage_A01",
    "actor_location": unreal.Vector(560.0, 535.0, 0.0),
    "actor_yaw": -42.0,
}
...
MAGE_SOCKET_SPECS = [
    ...
    ("vfx_sorcerer_focus", "head", unreal.Vector(12.0, 0.0, 10.0), unreal.Rotator(0.0, 0.0, 0.0)),
    ("vfx_worthiness_mark", "pelvis", unreal.Vector(12.0, 0.0, 6.0), unreal.Rotator(0.0, 0.0, 0.0)),
...
def ensure_materials():
    return {
        "M_INF_Mage_Skin_Blockout_A01": color_material(
            "M_INF_Mage_Skin_Blockout_A01",
            unreal.LinearColor(0.62, 0.11, 0.075, 1.0),
        ),
        "M_INF_Mage_HornClaw_Blockout_A01": color_material(
            "M_INF_Mage_HornClaw_Blockout_A01",
            unreal.LinearColor(0.025, 0.020, 0.018, 1.0),
            roughness=0.70,
        ),
        "M_INF_Mage_Wing_Blockout_A01": color_material(
            "M_INF_Mage_Wing_Blockout_A01",
            unreal.LinearColor(0.14, 0.045, 0.040, 1.0),
        ),
        "M_INF_Mage_RitualCloth_Blockout_A01": color_material(
            "M_INF_Mage_RitualCloth_Blockout_A01",
            unreal.LinearColor(0.075, 0.055, 0.052, 1.0),
        ),
        "M_INF_Mage_ObsidianArmor_Blockout_A01": color_material(
            "M_INF_Mage_ObsidianArmor_Blockout_A01",
            unreal.LinearColor(0.035, 0.032, 0.038, 1.0),
            roughness=0.62,
            metallic=0.45,
        ),
        "M_INF_Mage_BoneOrnaments_Blockout_A01": color_material(
            "M_INF_Mage_BoneOrnaments_Blockout_A01",
            unreal.LinearColor(0.58, 0.49, 0.35, 1.0),
        ),
        "M_INF_Mage_AbyssalGlow_Blockout_A01": color_material(
            "M_INF_Mage_AbyssalGlow_Blockout_A01",
...
def material_instance_name(mesh_name, material_name):
    ...
    for prefix in ("M_INF_Mage_", "M_INF_"):
...
def assign_project_materials(mesh, project_materials):
    ...
    unreal.log("Synced {} Infernal Mage material slots for {}".format(len(slots), mesh.get_name()))
...
def import_skeletal_mesh(entry):
    ...
    unreal.EditorAssetLibrary.save_asset(asset_path)
    unreal.log("Imported Infernal Mage skeletal mesh: {}".format(asset_path))
    return mesh
...
def ensure_skeletal_mesh_sockets(mesh):
    for socket_name, _bone_name, _location, _rotation in MAGE_SOCKET_SPECS:
        remove_socket_name(mesh, socket_name)
        remove_socket_name(mesh, "SOCKET_{}".format(socket_name))

    for socket_name, bone_name, location, rotation in MAGE_SOCKET_SPECS:
    ...
    unreal.EditorAssetLibrary.save_loaded_asset(mesh)
    unreal.log("Ensured {} Infernal Mage sockets on {}".format(len(MAGE_SOCKET_SPECS), mesh.get_name()))
...
def tag_actor(actor):
    ...
    for tag in (unreal.Name(MAGE_TAG), unreal.Name("AET_NEXT_SLICE")):
        if tag not in tags:
            tags.append(tag)
...
def update_startup_level(mesh):
    ...
    spawn_skeletal_mesh(
        MAGE_ASSET["actor_label"],
        mesh,
        MAGE_ASSET["actor_location"],
        MAGE_ASSET["actor_yaw"],
    )
    ...
    unreal.log("Updated startup level with Infernal Mage review actor.")


def main():
    materials = ensure_materials()
    mesh = import_skeletal_mesh(MAGE_ASSET)
    assign_project_materials(mesh, materials)
    ensure_physics_asset(mesh, MAGE_ASSET["physics_asset"])
    ensure_review_lods_skeletal(mesh)
    ensure_skeletal_mesh_sockets(mesh)
    ensure_anim_blueprint(
        MAGE_ASSET["anim_blueprint"],
        MAGE_ASSET["skeleton"],
        "{}/{}".format(MAGE_ASSET["destination"], MAGE_ASSET["name"]),
    )
    update_startup_level(mesh)
    unreal.log("Aerathea Infernal Mage import complete.")
