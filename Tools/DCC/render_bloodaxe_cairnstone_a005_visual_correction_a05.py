#!/usr/bin/env python3
"""Render A005 A05 internal proofs or the single eligible final review."""
...
A05_BLEND = ROOT / "SourceAssets/Blender/Props/Giants/BloodAxe/Cairns/SM_GIA_BloodAxeCairnstone_A005/SM_GIA_BloodAxeCairnstone_A005_DCCGameReady_VisualCorrection_A05.blend"
...
def shadeless_material(bpy: Any, source_material: Any) -> Any:
    material = bpy.data.materials.new("M_A005_A05_INTERNAL_SHADELESS")
    ...
    source_node = next(
        ...
        if node.bl_idname == "ShaderNodeTexImage" and node.name == "A05_DIRECT_SOURCE_BASECOLOR"
    )
...
def main() -> int:
    ...
    with bpy.data.libraries.load(str(A05_BLEND), link=False) as (data_from, data_to):
    ...
    if lod0 is None:
        raise RuntimeError("A05 LOD0 missing")
    ...
    front_camera = helper.make_camera(
        bpy, "A005_A05_FRONT_ORTHO", (0.0, -500.0, 110.0), (0.0, 0.0, 110.0), ortho_scale=250.0
    )
    front_shadeless_rel = proof_root / "A05_FRONT_SHADELESS_RGBA.png"
    ...
    left_camera = helper.make_camera(
        bpy, "A005_A05_LEFT_ORTHO", (-500.0, 0.0, 110.0), (0.0, 0.0, 110.0), ortho_scale=250.0
    )
    left_shadeless_rel = proof_root / "A05_LEFT_SHADELESS_RGBA.png"
    ...
    helper.configure_scene(scene, (1024, 1024))
    helper.remove_lights(bpy)
    helper.area_light(bpy, "A005_A05_PROOF_FRONT", (0.0, -420.0, 145.0), 1950000.0, 420.0)
    helper.area_light(bpy, "A005_A05_PROOF_FILL", (-220.0, -260.0, 250.0), 480000.0, 360.0)
    front_lit_camera = helper.make_camera(
        bpy, "A005_A05_FRONT_LIT", (0.0, -500.0, 110.0), (0.0, 0.0, 110.0), ortho_scale=250.0
    )
    front_lit_rel = proof_root / "A05_FRONT_LIT_RGBA.png"
    ...
    helper.remove_lights(bpy)
    helper.area_light(bpy, "A005_A05_FINAL_FRONT", (0.0, -470.0, 165.0), 2200000.0, 430.0)
    helper.area_light(bpy, "A005_A05_FINAL_KEY", (-250.0, -330.0, 285.0), 1050000.0, 300.0)
    helper.area_light(bpy, "A005_A05_FINAL_FILL", (260.0, -350.0, 210.0), 260000.0, 380.0)
    helper.area_light(bpy, "A005_A05_FINAL_RIM", (180.0, 250.0, 300.0), 80000.0, 280.0)
    final_camera = helper.make_camera(
        bpy, "A005_A05_FINAL_REVIEW", (190.0, -560.0, 225.0), (0.0, 0.0, 102.0), lens=72.0
    )
    if args.mode == "final":
        ...
        rgba_rel = attempt_root / "A05_ATTEMPT_OBJECT_RGBA.png"
        image_rel = attempt_root / "A05_ATTEMPT.png"
        audit_rel = attempt_root / "A05_ATTEMPT_AUDIT.json"
    ...
    audit = {
        "schema": "aerathea.a005_visual_correction_a05_render_audit.v1",
        "asset_id": ASSET,
        "contract_id": CONTRACT,
        "status": "render_complete_pending_a05_independent_audit",
        "mode": args.mode,
        "path": str(image_rel),
