#!/usr/bin/env python3
"""Build the backside-repaired 360 Blood Axe cairn slab cluster candidate.

Run with:
    blender --background --python Tools/DCC/build_bloodaxe_cairn_a1_backside_repair.py

This is the repair pass after Unreal review showed the prior A1/Test2Manual
backside was not acceptable. It keeps the approved Blood Axe cairn front read,
but rebuilds the rear as actual 360-degree stone volume with side/back
treatment, painted Blood Axe marks, rawhide bindings, texture sets, LOD source
exports, collision proxy, and review renders.
"""
...
REVIEW_ROOT = ROOT / "Saved" / "Automation" / "DCC" / "SM_GIA_BloodAxeCairnSlabCluster_A01_BacksideRepair"

ASSET_NAME = "SM_GIA_BloodAxeCairnSlabCluster_A01_BacksideRepair"
...
def generate_texture_set(label: str, base: tuple[float, float, float], accent: tuple[float, float, float], seed: int) -> dict[str, Path]:
    ...
    paths = {
        "bc": texture_dir / f"T_GIA_BloodAxeCairnSlabCluster_A01_BacksideRepair_{label}_BC.png",
        "n": texture_dir / f"T_GIA_BloodAxeCairnSlabCluster_A01_BacksideRepair_{label}_N.png",
        "orm": texture_dir / f"T_GIA_BloodAxeCairnSlabCluster_A01_BacksideRepair_{label}_ORM.png",
    }
...
def generate_textures() -> dict[str, dict[str, Path]]:
    return {
        "stone": generate_texture_set("Stone", (0.18, 0.18, 0.155), (0.38, 0.35, 0.28), 1103),
        "earth": generate_texture_set("EarthAsh", (0.20, 0.14, 0.09), (0.49, 0.34, 0.20), 2207),
        "rawhide": generate_texture_set("Rawhide", (0.38, 0.20, 0.075), (0.78, 0.47, 0.19), 3301),
        "red": generate_texture_set("BloodAxeRedPaint", (0.66, 0.055, 0.038), (1.0, 0.19, 0.08), 4409),
    }
...
def make_materials(texture_paths: dict[str, dict[str, Path]]) -> dict[str, bpy.types.Material]:
    return {
        "stone": make_texture_material("M_GIA_BloodAxeCairn_BacksideRepairStone_A01", texture_paths["stone"], 0.9),
        "earth": make_texture_material("M_GIA_BloodAxeCairn_BacksideRepairEarthAsh_A01", texture_paths["earth"], 0.94),
        "rawhide": make_texture_material("M_GIA_BloodAxeCairn_BacksideRepairRawhide_A01", texture_paths["rawhide"], 0.86),
        "red": make_texture_material("M_GIA_BloodAxeCairn_BacksideRepairBloodAxeRedPaint_A01", texture_paths["red"], 0.78),
    }
...
def build_asset_lod(collection: bpy.types.Collection, materials: dict[str, bpy.types.Material], lod: int) -> list[bpy.types.Object]:
    ...
    detail = lod == 0
    mid = lod <= 1
    base_segments = 34 if detail else 18 if lod <= 2 else 12
    scuff_segments = 24 if detail else 14 if lod <= 2 else 8

    objects.append(add_irregular_disc(f"{prefix}_AshMudGroundingMound", collection, materials["earth"], 185.0, 128.0, 9.0, base_segments, 100 + lod))
    objects.append(add_irregular_disc(f"{prefix}_FrontMudAndStoneScuff", collection, materials["earth"], 122.0, 72.0, 6.0, scuff_segments, 120 + lod))
    objects[-1].location = (-18.0, -32.0, 5.0)
    objects.append(add_irregular_disc(f"{prefix}_BackMudAndStoneScuff", collection, materials["earth"], 128.0, 78.0, 6.5, scuff_segments, 140 + lod))
    objects[-1].location = (14.0, 78.0, 5.0)
    ...
    slab_specs = [
        ...
        ("BacksideCounterweightSlab", (8, 91, 58), (172, 54, 36), (math.radians(-4), math.radians(4), math.radians(5)), 221),
        ("BackLeftButtressStone", (-104, 84, 44), (90, 48, 56), (math.radians(-8), math.radians(-5), math.radians(-13)), 222),
        ("BackRightButtressStone", (112, 80, 48), (92, 46, 62), (math.radians(5), math.radians(7), math.radians(12)), 223),
        ("RearGroundLockWedge", (18, 120, 27), (154, 38, 28), (math.radians(2), math.radians(-3), math.radians(-2)), 224),
    ]
    if detail:
        slab_specs.extend(
            [
                ...
                ("RearBrokenMarkerShard", (-56, 126, 43), (42, 29, 72), (math.radians(-10), math.radians(8), math.radians(-19)), 225),
                ("RearSmallRubbleCap", (82, 119, 33), (56, 30, 35), (math.radians(7), math.radians(-5), math.radians(24)), 226),
            ]
        )
    elif mid:
        slab_specs.extend(
            [
                ...
                ("RearBrokenMarkerShard", (-56, 126, 39), (36, 26, 58), (math.radians(-10), math.radians(8), math.radians(-19)), 225),
            ]
        )
    ...
    if mid:
        paint_specs = [
            ...
            ("BacksideOxideSlashA", (2, 119, 73), (94, 7, 2.2), (math.radians(-4), math.radians(4), math.radians(17)), 307),
            ("BacksideOxideSlashB", (-30, 117, 68), (70, 7, 2.0), (math.radians(-4), math.radians(4), math.radians(-31)), 308),
            ("BacksideButtressMark", (108, 107, 64), (54, 6, 2.0), (math.radians(4), math.radians(7), math.radians(42)), 309),
        ]
    ...
    if detail:
        rawhide_specs = [
            ...
            ("RearCounterweightBinding", (-28, 119, 60), (94, 7, 5), (math.radians(-4), math.radians(4), math.radians(79)), 404),
            ("RearButtressLashing", (96, 108, 56), (64, 6, 5), (math.radians(5), math.radians(7), math.radians(101)), 405),
        ]
        ...
        pebble_specs = [
            ...
            (8, 92, 18, (18, 13, 9), 508),
            (-126, 112, 17, (21, 14, 10), 509),
            (-52, 141, 16, (17, 12, 8), 510),
            (46, 151, 16, (19, 13, 9), 511),
            (132, 116, 18, (22, 14, 10), 512),
        ]
...
def configure_review_scene() -> tuple[bpy.types.Object, Vector]:
    ...
    bpy.ops.object.light_add(type="AREA", location=(260, 320, 250))
...
def add_metadata_to_objects(objects: list[bpy.types.Object]) -> None:
    add_asset_metadata(
        ASSET_NAME,
        "BacksideRepair DCC game-ready candidate pending Unreal import validation and Flamestrike approval",
        UNREAL_PATH,
    )
    for obj in objects:
        ...
        obj["Aerathea.Status"] = "dcc_game_ready_candidate_pending_unreal_import_and_flamestrike_review"
        obj["Aerathea.SourceMethod"] = "hand_authored_360_dcc_backside_repair_from_approved_a1_concept_direction"
        obj["Aerathea.Collision"] = "UCX broad hull updated for corrected rear stone depth"
...
def main() -> None:
    ...
    compose_review_board(render_paths, REVIEW_ROOT / f"{ASSET_NAME}_BacksideRepairTurntableReview.png", 960, 540)
