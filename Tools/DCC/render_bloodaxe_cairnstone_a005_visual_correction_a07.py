#!/usr/bin/env python3
"""Render A005 A07 internal proofs or the single eligible final review."""
...
A07_BLEND = ROOT / "SourceAssets/Blender/Props/Giants/BloodAxe/Cairns/SM_GIA_BloodAxeCairnstone_A005/SM_GIA_BloodAxeCairnstone_A005_DCCGameReady_VisualCorrection_A07.blend"
...
def shadeless_material(bpy: Any, source_material: Any) -> Any:
    material = bpy.data.materials.new("M_A005_A07_INTERNAL_SHADELESS")
    ...
    source_node = next(
        ...
        if node.bl_idname == "ShaderNodeTexImage" and node.name == "A07_DIRECT_SOURCE_BASECOLOR"
    )
...
def main() -> int:
    ...
    with bpy.data.libraries.load(str(A07_BLEND), link=False) as (data_from, data_to):
    ...
    if lod0 is None:
        raise RuntimeError("A07 LOD0 missing")
    ...
    front_camera = helper.make_camera(
        bpy, "A005_A07_FRONT_ORTHO", (0.0, -500.0, 110.0), (0.0, 0.0, 110.0), ortho_scale=250.0
    )
    front_shadeless_rel = proof_root / "A07_FRONT_SHADELESS_RGBA.png"
    ...
    left_camera = helper.make_camera(
        bpy, "A005_A07_LEFT_ORTHO", (-500.0, 0.0, 110.0), (0.0, 0.0, 110.0), ortho_scale=250.0
    )
    left_shadeless_rel = proof_root / "A07_LEFT_SHADELESS_RGBA.png"
    ...
    top_camera = helper.make_camera(
        bpy, "A005_A07_TOP_ORTHO", (0.0, 0.0, 500.0), (0.0, 0.0, 0.0), ortho_scale=170.0
    )
    top_shadeless_rel = proof_root / "A07_TOP_SHADELESS_RGBA.png"
    top_shadeless = helper.render_png(bpy, top_shadeless_rel)
    bpy.data.objects.remove(top_camera, do_unlink=True)

    lod0.data.materials[0] = source_material
    helper.configure_scene(scene, (1024, 1024))
    helper.remove_lights(bpy)
    helper.area_light(bpy, "A005_A07_PROOF_FRONT", (0.0, -420.0, 145.0), 1950000.0, 420.0)
    helper.area_light(bpy, "A005_A07_PROOF_FILL", (-220.0, -260.0, 250.0), 480000.0, 360.0)
    front_lit_camera = helper.make_camera(
        bpy, "A005_A07_FRONT_LIT", (0.0, -500.0, 110.0), (0.0, 0.0, 110.0), ortho_scale=250.0
    )
    front_lit_rel = proof_root / "A07_FRONT_LIT_RGBA.png"
    ...
    helper.remove_lights(bpy)
    helper.area_light(bpy, "A005_A07_FINAL_FRONT", (0.0, -470.0, 165.0), 2200000.0, 430.0)
    helper.area_light(bpy, "A005_A07_FINAL_KEY", (-250.0, -330.0, 285.0), 1050000.0, 300.0)
    helper.area_light(bpy, "A005_A07_FINAL_FILL", (260.0, -350.0, 210.0), 260000.0, 380.0)
    helper.area_light(bpy, "A005_A07_FINAL_RIM", (180.0, 250.0, 300.0), 80000.0, 280.0)
    final_camera = helper.make_camera(
        bpy, "A005_A07_FINAL_REVIEW", (190.0, -560.0, 225.0), (0.0, 0.0, 102.0), lens=72.0
    )
    if args.mode == "final":
        ...
        rgba_rel = attempt_root / "A07_ATTEMPT_OBJECT_RGBA.png"
        image_rel = attempt_root / "A07_ATTEMPT.png"
        audit_rel = attempt_root / "A07_ATTEMPT_AUDIT.json"
    ...
    audit = {
        "schema": "aerathea.a005_visual_correction_a07_render_audit.v1",
        "asset_id": ASSET,
        "contract_id": CONTRACT,
        "status": "render_complete_pending_a07_independent_audit",
        "mode": args.mode,
        "path": str(image_rel),
        "object_rgba_path": str(rgba_rel),
        "size": list(final.size),
        "sha256": helper.sha256_file(ROOT / image_rel),
        "orientation": "upright source-oriented front three-quarter; complete oval replacement base and expanded top-visible masonry crowns visible",
        ...
        "proofs": {
            ...
            "top_shadeless": {
                "path": str(top_shadeless_rel),
                "sha256": helper.sha256_file(ROOT / top_shadeless_rel),
                "alpha_bounds": helper.alpha_bounds(top_shadeless),
                "camera": "true orthographic +Z view; image x=+X and image y=-Y",
                "ortho_scale_cm": 170.0,
            },
            "front_lit": {
