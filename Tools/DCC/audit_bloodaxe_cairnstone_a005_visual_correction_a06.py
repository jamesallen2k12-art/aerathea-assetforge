#!/usr/bin/env python3
"""Independently audit the A005 A06 measured oval-base correction."""
...
VALIDATION_REL = BLUEPRINT_ROOT / "manifests/VISUAL_CORRECTION_A06_VALIDATION.json"
DCC_ROOT = Path("SourceAssets/Blender/Props/Giants/BloodAxe/Cairns") / ASSET
A06_BLEND_REL = DCC_ROOT / f"{ASSET}_DCCGameReady_VisualCorrection_A06.blend"
...
A06_BC_REL = Path("SourceAssets/Textures/Props/Giants/BloodAxe/Cairns") / ASSET / "VisualCorrection_A06/T_GIA_BloodAxeCairnstone_A005_VF_A06_BC.png"
...
def source_owned_rgb_audit() -> Dict[str, Any]:
    ...
    candidate = Image.open(ROOT / A06_BC_REL).convert("RGB")
    compared = 0
    mismatches = 0
...
def main() -> int:
    ...
    candidate_lods = {
        lod_name: load_object_from_blend(bpy, A06_BLEND_REL, f"{ASSET}_{lod_name}")
    ...
    plinth_signatures = {
        ...
        "A06": component_signature(lod0, plinth["indices"]),
    }
    ...
    collisions = []
    with bpy.data.libraries.load(str(ROOT / A06_BLEND_REL), link=False) as (data_from, data_to):
    ...
    gate("G08_exact_A04_plinth_geometry_and_uv", plinth_signatures["A04"] == plinth_signatures["A06"], plinth_signatures)
    gate("G09_material_and_uv", all(lods[name]["materials"] == 1 and lods[name]["uv_layers"] == ["UVMap", "LightmapUV"] for name in lods) and all(name in material_nodes for name in ("A06_DIRECT_SOURCE_BASECOLOR", "A06_DIRECTX_NORMAL", "A06_ORM_NONMETALLIC")), {"lods": {name: {"materials": lods[name]["materials"], "uv_layers": lods[name]["uv_layers"]} for name in lods}, "material_nodes": material_nodes})
    ...
    status = (
        ...
        if complete
        else "blocked_a06_independent_audit"
    )
    validation = {
        "schema": "aerathea.a005_visual_correction_a06_validation.v1",
        "asset_id": ASSET,
