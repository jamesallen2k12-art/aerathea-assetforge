#!/usr/bin/env python3
"""Build the first-pass Aerathea Infernal Rogue review source.

Run with:
    blender --background --python Tools/DCC/build_infernal_rogue.py

This creates deterministic review geometry for the approved third Infernal
...
REVIEW_ROOT = ROOT / "Saved" / "Automation" / "InfernalRogueReview"
...
ASSET_NAME = "SK_INF_Rogue_A01"
HEIGHT_CM = 168.0
SOURCE_REL = "Characters/Infernals/Rogue/SK_INF_Rogue_A01"
EXPORT_REL = "Characters/Infernals/Rogue/SK_INF_Rogue_A01"

INF_ROGUE_MATERIALS = {
    "M_INF_Rogue_Skin_Blockout_A01": (0.52, 0.075, 0.060),
    "M_INF_Rogue_HornClaw_Blockout_A01": (0.025, 0.020, 0.018),
    "M_INF_Rogue_Wing_Blockout_A01": (0.10, 0.035, 0.040),
    "M_INF_Rogue_Wraps_Blockout_A01": (0.035, 0.034, 0.037),
    "M_INF_Rogue_LightArmor_Blockout_A01": (0.028, 0.026, 0.032),
    "M_INF_Rogue_BoneHorn_Blockout_A01": (0.50, 0.43, 0.32),
    "M_INF_Rogue_SightGlow_Blockout_A01": (0.72, 0.06, 1.00),
...
def add_skull_belt(label: str, armature: bpy.types.Object, objects: list[bpy.types.Object], materials: dict[str, bpy.types.Material]) -> None:
    height = HEIGHT_CM
    bone = materials["M_INF_Rogue_BoneHorn_Blockout_A01"]
    armor = materials["M_INF_Rogue_LightArmor_Blockout_A01"]
...
def add_infernal_rogue(
    ...
    shoulder = height * 0.104
    hip = height * 0.052
    wing_span = height * 0.34
    tail_len = height * 0.56

    armature = create_armature(armature_name, make_bones(height))
    objects: list[bpy.types.Object] = []

    skin = materials["M_INF_Rogue_Skin_Blockout_A01"]
    horn = materials["M_INF_Rogue_HornClaw_Blockout_A01"]
    wing = materials["M_INF_Rogue_Wing_Blockout_A01"]
    cloth = materials["M_INF_Rogue_Wraps_Blockout_A01"]
    armor = materials["M_INF_Rogue_LightArmor_Blockout_A01"]
    bone = materials["M_INF_Rogue_BoneHorn_Blockout_A01"]
    glow = materials["M_INF_Rogue_SightGlow_Blockout_A01"]
    ...
    for side, sign, suffix in (("L", 1, "l"), ("R", -1, "r")):
        ...
        add_assigned(add_diamond_obj(f"{label}_InvisibleSight_EyeGlow_{side}", (height * 0.080, sign * height * 0.013, height * 0.915), (1.4, 1.6, 2.2), glow), armature, "head", objects)
        ...
        add_assigned(add_box_obj(f"{label}_RogueBracer_{side}_LightClawGuard", (height * 0.115, sign * shoulder * 1.42, height * 0.43), (height * 0.030, height * 0.042, height * 0.052), armor), armature, f"lowerarm_{suffix}", objects)
        add_assigned(add_box_obj(f"{label}_RogueBracer_{side}_SightBrand", (height * 0.134, sign * shoulder * 1.44, height * 0.43), (height * 0.009, height * 0.010, height * 0.060), glow), armature, f"lowerarm_{suffix}", objects)
        ...
        add_assigned(add_ellipsoid_obj(f"{label}_ClawAmbushGlow_{side}", (height * 0.232, sign * shoulder * 1.48, height * 0.325), (3.2, 3.2, 3.2), glow, 8, 4), armature, f"hand_{suffix}", objects)
        ...
        add_assigned(add_diamond_obj(f"{label}_WingRootSightMark_{side}", (-height * 0.010, sign * shoulder * 0.60, height * 0.69), (1.8, 2.2, 3.0), glow), armature, f"wing_{suffix}", objects)
    ...
    add_assigned(add_diamond_obj(f"{label}_TailTipSightGlow", (-tail_len, 0, height * 0.18), (2.5, 2.0, 2.7), glow), armature, "tail_02", objects)
    ...
    add_assigned(add_diamond_obj(f"{label}_AmbushMark_Chest", (height * 0.106, 0, height * 0.708), (height * 0.007, height * 0.014, height * 0.024), glow), armature, "chest", objects)
    ...
    add_assigned(add_box_obj(f"{label}_Wraps_Front", (height * 0.070, 0, height * 0.33), (height * 0.014, shoulder * 0.62, height * 0.190), cloth), armature, "pelvis", objects)
    add_assigned(add_box_obj(f"{label}_Wraps_Back", (-height * 0.040, 0, height * 0.35), (height * 0.012, shoulder * 0.56, height * 0.170), cloth), armature, "pelvis", objects)
    for side, sign in (("L", 1), ("R", -1)):
        add_assigned(add_box_obj(f"{label}_Wraps_Side_{side}", (height * 0.018, sign * shoulder * 0.52, height * 0.35), (height * 0.012, height * 0.022, height * 0.155), cloth), armature, "pelvis", objects)
    ...
    sockets = [
        ...
        ("SOCKET_vfx_ambush_mark", (height * 0.110, 0, height * 0.708), "chest"),
        ("SOCKET_vfx_invisible_sight_focus", (height * 0.086, 0, height * 0.918), "head"),
        ("SOCKET_pounce_trace", (height * 0.238, 0, height * 0.39), "chest"),
        ("SOCKET_claw_rake_trace", (height * 0.258, 0, height * 0.30), "chest"),
        ("SOCKET_tail_balance_trace", (-tail_len * 0.80, 0, height * 0.22), "tail_02"),
        ("SOCKET_crouch_center", (height * 0.07, 0, height * 0.43), "pelvis"),
    ]
...
def export_asset() -> None:
    clear_scene()
    setup_scene()
    materials = create_materials(INF_ROGUE_MATERIALS)
    armature, objects = add_infernal_rogue(ASSET_NAME, f"SKEL_{ASSET_NAME}", materials)
    add_asset_metadata(
        ASSET_NAME,
        "First-pass Infernal Rogue class overlay with swept horns, tight folded wings, close-fit wraps, light claw guards, wing strap anchors, tail-root armor, invisible-sight glow, ambush mark, and stealth trace sockets; final sculpt, retopo, skinning, physics, LODs, animation, and textures pending",
...
def render_review() -> None:
    ...
    add_infernal_rogue(ASSET_NAME, f"SKEL_{ASSET_NAME}", materials)
    add_scale_marker("Scale_Humanoid_180cm", 180.0, -130.0, materials["M_AET_ScaleMarker_Humanoid_A01"])
    add_label("Humanoid 180cm", (0, -130.0, 14.0), 10.0)
    add_scale_marker("Scale_CompactBand_173cm", 173.0, 130.0, materials["M_AET_ScaleMarker_Blockout_A01"])
    add_label("Compact band 173cm", (0, 130.0, 14.0), 10.0)
    add_asset_metadata(
        ASSET_NAME,
        "Approved Infernal Rogue first-pass DCC review scene with 168 cm Compact-band stealth ambusher, 180 cm humanoid marker, and 173 cm Compact-band marker",
        "/Game/Aerathea/Characters/Infernals/Rogue/SK_INF_Rogue_A01",
    )

    bpy.ops.object.light_add(type="AREA", location=(520, -360, 430))
    key = bpy.context.object
    key.name = "AET_InfernalRogueReview_KeyLight"
    key.data.energy = 7200
    key.data.size = 560
    bpy.ops.object.light_add(type="AREA", location=(380, 350, 300))
    fill = bpy.context.object
    fill.name = "AET_InfernalRogueReview_FillLight"
    fill.data.energy = 3400
    fill.data.size = 700
    bpy.ops.object.light_add(type="SUN", location=(0, 0, 500), rotation=(math.radians(52), 0, math.radians(35)))
    sun = bpy.context.object
    sun.name = "AET_InfernalRogueReview_SunFill"
    sun.data.energy = 1.15

    bpy.ops.object.camera_add(location=(680, -540, 280))
    camera = bpy.context.object
    target = Vector((12, 0, 100))
