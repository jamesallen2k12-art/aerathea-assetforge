#!/usr/bin/env python3
"""Build A1 Blood Axe cairn as a sculpted mid-poly DCC candidate, pass 08.

Run with:
    Tools/Blender/blender-4.5.11-linux-x64/blender --background --python Tools/DCC/build_bloodaxe_cairn_a1_sculpted_midpoly_v2.py

This pass corrects the shape-language failure in the first sculpted pass:
rectangular cuboid slabs are replaced with irregular prism/profile stones so the
asset reads closer to the approved compact A1 cairn from all turnaround angles.
"""
...
REVIEW_ROOT = ROOT / "Saved" / "Automation" / "DCC" / "SM_GIA_BloodAxeCairnSlabCluster_A01_SculptedMidpolyV2"

ASSET_NAME = "SM_GIA_BloodAxeCairnSlabCluster_A01_SculptedMidpolyV2"
...
def make_materials() -> dict[str, bpy.types.Material]:
    return {
        "stone": make_material("M_GIA_BloodAxeCairn_SculptedV2_Stone_A01", (0.53, 0.500, 0.410), 0.92, 0.20),
        "stone_dark": make_material("M_GIA_BloodAxeCairn_SculptedV2_DarkStone_A01", (0.34, 0.315, 0.255), 0.94, 0.15),
        ...
        "earth": make_material("M_GIA_BloodAxeCairn_SculptedV2_AshMud_A01", (0.39, 0.260, 0.145), 0.98, 0.12),
        "rawhide": make_material("M_GIA_BloodAxeCairn_SculptedV2_Rawhide_A01", (0.82, 0.505, 0.195), 0.82, 0.20),
        "paint": make_material("M_GIA_BloodAxeCairn_SculptedV2_OxideRed_A01", (0.70, 0.072, 0.050), 0.88, 0.18),
...
def build_lod(col: bpy.types.Collection, mats: dict[str, bpy.types.Material], detail: int) -> list[bpy.types.Object]:
    ...
    central = add_irregular_prism(
        ...
        21.0,
        (0.0, -15.0, 48.0),
        (math.radians(2.0), math.radians(-16.0), math.radians(-8.5)),
        100 + detail,
        1.2,
        2.2,
    )
    ...
    under_front = add_irregular_prism(
        f"A1_Sculpted_FrontBrokenSupportSlab_LOD{detail}",
        col,
        mats["stone_dark"],
        [(-82, -18), (-31, -25), (34, -20), (83, -11), (76, 12), (26, 21), (-30, 18), (-88, 6)],
        15.0,
        (4.0, -55.0, 29.5),
    ...
    if detail <= 2:
        left_cap = add_irregular_prism(
            ...
            col,
            mats["stone"],
            [(-43, -16), (-8, -20), (38, -14), (45, 7), (17, 18), (-31, 15), (-47, 1)],
            15.0,
            (-108.0, -44.0, 46.0),
    ...
    right_front = add_irregular_prism(
        ...
        col,
        mats["stone"],
        [(-34, -15), (3, -20), (36, -12), (42, 8), (10, 20), (-29, 16), (-42, 1)],
        16.0,
        (106.0, -50.0, 30.0),
    ...
    if include_paint:
        objects.extend(
            [
                add_top_decal(central, f"A1_Sculpted_MainRedLongStroke_LOD{detail}", col, mats["paint"], (-8, -2), (148, 12), math.radians(18), 11.30),
                add_top_decal(central, f"A1_Sculpted_MainRedCrossA_LOD{detail}", col, mats["paint"], (-45, 14), (78, 10), math.radians(-31), 11.46),
                add_top_decal(central, f"A1_Sculpted_MainRedCrossB_LOD{detail}", col, mats["paint"], (35, 7), (72, 9), math.radians(-24), 11.58),
    ...
    if include_detail:
        objects.extend(add_stone_details(central, "A1_Sculpted_Main", col, mats, 11.82, 228, 70, 700, 36))
    ...
    if include_rope:
        objects.extend(add_lashing_band(central, f"A1_Sculpted_LeftRawhideBind_LOD{detail}", col, mats["rawhide"], -78.0, 70.0, 13.0))
        objects.extend(add_lashing_band(central, f"A1_Sculpted_RightRawhideBind_LOD{detail}", col, mats["rawhide"], 76.0, 70.0, 13.0))
        objects.extend(add_lashing_band(left_low, f"A1_Sculpted_LeftStackRawhideBind_LOD{detail}", col, mats["rawhide"], 4.0, 42.0, 10.5))
