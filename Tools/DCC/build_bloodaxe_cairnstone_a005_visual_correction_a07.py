#!/usr/bin/env python3
"""Build the A005 A07 measured oval replacement-base candidate."""
...
TEXTURE_ROOT = TEXTURE_PARENT / "VisualCorrection_A07"
BC_REL = TEXTURE_ROOT / "T_GIA_BloodAxeCairnstone_A005_VF_A07_BC.png"
N_REL = TEXTURE_ROOT / "T_GIA_BloodAxeCairnstone_A005_VF_A07_N.png"
ORM_REL = TEXTURE_ROOT / "T_GIA_BloodAxeCairnstone_A005_VF_A07_ORM.png"
DCC_ROOT = Path("SourceAssets/Blender/Props/Giants/BloodAxe/Cairns") / ASSET
BLEND_REL = DCC_ROOT / f"{ASSET}_DCCGameReady_VisualCorrection_A07.blend"
...
def source_panel_y_a07(view: str, z_value: float) -> float:
    mappings = {
...
def base_half_extent_a07(z_value: float, axis: str) -> float:
    if z_value <= 10.5:
...
def configure_base(base: Any, a04: Any) -> None:
        ...
        if name == "LOWER_COURSE_CORE":
            result = masonry_course_shell(
                ...
                (
                    ...
                    (24.2, 137.307692, 105.196507, "side"),
                    (25.5, 137.307692, 105.196507, "top"),
        ...
        if name == "UPPER_COURSE_CORE":
            result = masonry_course_shell(
                ...
                (
                    ...
                    (33.8, 123.846154, 92.707424, "side"),
                    (35.0, 123.846154, 92.707424, "top"),
                    (35.0, 96.0, 68.0, "top"),
                ),
                70502,
                collection,
    ...
    def project_point_a07(point: Vec3, view: str, bbox: Sequence[int], row_spans: Sequence[Tuple[int, int]]):
        ...
        if view in ("front", "back", "left", "right"):
            panel_y = source_panel_y_a07(view, max(0.0, min(220.0, z_value))) if z_value < 34.0 else a04.source_panel_y(view, z_value)
            horizontal = {"front": x_value, "back": -x_value, "left": -y_value, "right": y_value}[view]
            axis = "x" if view in ("front", "back") else "y"
            half_extent = a04.interpolate_profile(z_value, axis) if z_value >= 34.0 else base_half_extent_a07(z_value, axis)
    ...
    def create_materials_a07(bpy: Any):
        stone, removable = a04_materials(bpy)
        stone.name = "M_GIA_BloodAxeCairnstone_A005_VisualCorrection_A07"
        removable.name = "M_GIA_BloodAxeCairnstone_A005_A07_REMOVED_HELPER"
        for node in stone.node_tree.nodes:
            if node.name == "A04_DIRECT_SOURCE_BASECOLOR":
                node.name = "A07_DIRECT_SOURCE_BASECOLOR"
            elif node.name == "A04_DIRECTX_NORMAL":
                node.name = "A07_DIRECTX_NORMAL"
            elif node.name == "A04_ORM_NONMETALLIC":
                node.name = "A07_ORM_NONMETALLIC"
        stone["aerathea_display_color_chain"] = "A07 component-local exact source RGB; no gamma or grading"
        return stone, removable

    def assign_uv_a07(obj: Any) -> Dict[str, int]:
        # Capture A07's source-panel routing for the replacement base, then
        ...
        if source_layer is None:
            raise RuntimeError("A07 source UVMap missing")
        ...
        if layer is None:
            raise RuntimeError("A07 UVMap missing")
        ...
        # Restore A07 source-panel ownership only on the three replacement
        ...
        obj["aerathea_A07_structure"] = "preserved_A04_plinth_on_measured_oval_upper_and_lower_masonry_courses"
        obj["aerathea_A07_component_footprints_cm"] = json.dumps(
            {"C002": [123.846154, 92.707424], "C003": [137.307692, 105.196507], "C004": [140.0, 110.0]}
        )
        obj["aerathea_A07_top_visible_crowns_cm"] = json.dumps(
            {"C002": [123.846154, 92.707424], "C003": [137.307692, 105.196507], "C004": [140.0, 110.0]}
        )
        obj["aerathea_A07_A06_geometry_input_count"] = 0
        obj["aerathea_A07_component_local_top_uv"] = True
        obj["aerathea_A07_C004_authored_stone_faces"] = c004_authored_faces
        obj["aerathea_A07_color_grading"] = False
        return counts

    base.loft = loft_a07
    base.project_point = project_point_a07
    base.create_materials = create_materials_a07
    base.assign_source_uv = assign_uv_a07
...
def schema_report() -> Dict[str, Any]:
    ...
    passed = all(item["pass"] for item in checks.values()) and measurement["decision"] == "authoritative exterior-edge measurement record ready for A07 contract"
    return {
        "schema": "aerathea.a005_visual_correction_a07_builder_preflight.v1",
        "asset_id": ASSET,
...
def build() -> Dict[str, Any]:
    ...
    if preflight["status"] != "pass_schema_only_no_output":
        raise RuntimeError("A07 authority preflight blocked")
    base = load_module("a005_a07_packaging", BASE_MODULE_PATH)
    ...
    manifest.update(
        {
            "schema": "aerathea.a005_visual_correction_a07_candidate.v1",
            "contract_id": CONTRACT,
            "date": "2026-07-21",
            "status": "candidate_pending_a07_independent_audit",
            "artifact_classification": "candidate",
            "pipeline_status": "DCC game-ready candidate pending A07 audit and Flamestrike review",
            "recovery_from": "A06 visually rejected top-crown projection false pass; no A06 geometry input",
            "geometry_authority": "A07 source-derived exterior extents placed on top-visible crowns; exact A04 plinth; A06 geometry inputs zero",
            ...
            "a07_structure": {
                ...
                "C002": {"role": "upper exterior-measured oval masonry course", "footprint_cm": [123.846154, 92.707424], "top_visible_crown_outer_cm": [123.846154, 92.707424], "height_cm": 9.5, "blocks": 17, "oval_exponent": 2.0},
                "C003": {"role": "larger lower exterior-measured oval masonry course", "footprint_cm": [137.307692, 105.196507], "top_visible_crown_outer_cm": [137.307692, 105.196507], "height_cm": 15.0, "blocks": 19, "oval_exponent": 2.0},
                "C004": {"role": "shallow oval peripheral rubble apron", "footprint_cm": [140.0, 110.0], "top_visible_crown_outer_cm": [140.0, 110.0], "height_cm": 10.5, "blocks": 24, "oval_exponent": 2.0},
            },
            "replacement_base": {"A04_base_retained": False, "A04_base_hidden": False, "A04_base_geometry_inputs": 0, "A06_geometry_inputs": 0},
