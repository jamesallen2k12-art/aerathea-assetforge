#!/usr/bin/env python3
"""Build the first-pass Aerathea Infernal Mage review source.

Run with:
    blender --background --python Tools/DCC/build_infernal_mage.py

This creates deterministic review geometry for the approved first Infernal
...
REVIEW_ROOT = ROOT / "Saved" / "Automation" / "InfernalMageReview"
...
ASSET_NAME = "SK_INF_Mage_A01"
HEIGHT_CM = 203.0
SOURCE_REL = "Characters/Infernals/Mage/SK_INF_Mage_A01"
EXPORT_REL = "Characters/Infernals/Mage/SK_INF_Mage_A01"

INF_MAGE_MATERIALS = {
    "M_INF_Mage_Skin_Blockout_A01": (0.62, 0.11, 0.075),
    "M_INF_Mage_HornClaw_Blockout_A01": (0.025, 0.020, 0.018),
    "M_INF_Mage_Wing_Blockout_A01": (0.14, 0.045, 0.040),
    "M_INF_Mage_RitualCloth_Blockout_A01": (0.075, 0.055, 0.052),
    "M_INF_Mage_ObsidianArmor_Blockout_A01": (0.035, 0.032, 0.038),
    "M_INF_Mage_BoneOrnaments_Blockout_A01": (0.58, 0.49, 0.35),
    "M_INF_Mage_AbyssalGlow_Blockout_A01": (1.00, 0.24, 0.035),
...
def add_skull_belt(label: str, armature: bpy.types.Object, objects: list[bpy.types.Object], materials: dict[str, bpy.types.Material]) -> None:
    height = HEIGHT_CM
    bone = materials["M_INF_Mage_BoneOrnaments_Blockout_A01"]
    armor = materials["M_INF_Mage_ObsidianArmor_Blockout_A01"]
...
def add_infernal_mage(
    ...
    skin = materials["M_INF_Mage_Skin_Blockout_A01"]
    horn = materials["M_INF_Mage_HornClaw_Blockout_A01"]
    wing = materials["M_INF_Mage_Wing_Blockout_A01"]
    cloth = materials["M_INF_Mage_RitualCloth_Blockout_A01"]
    armor = materials["M_INF_Mage_ObsidianArmor_Blockout_A01"]
    bone = materials["M_INF_Mage_BoneOrnaments_Blockout_A01"]
    glow = materials["M_INF_Mage_AbyssalGlow_Blockout_A01"]

    body_parts = [
        (add_ellipsoid_obj(f"{label}_Pelvis", (0, 0, height * 0.45), (height * 0.055, hip * 1.25, height * 0.050), skin, 14, 7), "pelvis"),
        (add_ellipsoid_obj(f"{label}_Torso", (height * 0.02, 0, height * 0.62), (height * 0.073, shoulder * 0.82, height * 0.105), skin, 16, 8), "spine_01"),
        (add_ellipsoid_obj(f"{label}_Chest", (height * 0.02, 0, height * 0.72), (height * 0.082, shoulder * 1.05, height * 0.075), skin, 16, 8), "chest"),
    ...
    for side, sign, suffix in (("L", 1, "l"), ("R", -1, "r")):
        ...
        add_assigned(add_cylinder_between(f"{label}_UpperArm_{side}", (0, sign * shoulder, height * 0.72), (height * 0.06, sign * shoulder * 1.52, height * 0.56), height * 0.018, skin, 10), armature, f"upperarm_{suffix}", objects)
        add_assigned(add_cylinder_between(f"{label}_LowerArm_{side}", (height * 0.06, sign * shoulder * 1.52, height * 0.56), (height * 0.09, sign * shoulder * 1.62, height * 0.37), height * 0.016, skin, 10), armature, f"lowerarm_{suffix}", objects)
        add_assigned(add_box_obj(f"{label}_Bracer_{side}_BrandChannel", (height * 0.087, sign * shoulder * 1.58, height * 0.47), (height * 0.032, height * 0.048, height * 0.045), armor), armature, f"lowerarm_{suffix}", objects)
        add_assigned(add_box_obj(f"{label}_ForearmBrand_{side}", (height * 0.108, sign * shoulder * 1.60, height * 0.47), (height * 0.010, height * 0.010, height * 0.070), glow), armature, f"lowerarm_{suffix}", objects)
        ...
        add_assigned(add_ellipsoid_obj(f"{label}_HandCastGlow_{side}", (height * 0.19, sign * shoulder * 1.67, height * 0.36), (5.0, 5.0, 5.0), glow, 10, 5), armature, f"hand_{suffix}", objects)
        ...
        add_assigned(add_cylinder_between(f"{label}_Thigh_{side}", (0, sign * hip, height * 0.43), (height * 0.04, sign * hip * 1.20, height * 0.24), height * 0.023, skin, 10), armature, f"thigh_{suffix}", objects)
        add_assigned(add_cylinder_between(f"{label}_Calf_{side}", (height * 0.04, sign * hip * 1.20, height * 0.24), (height * 0.07, sign * hip * 1.15, height * 0.055), height * 0.018, skin, 10), armature, f"calf_{suffix}", objects)
    ...
    add_assigned(add_box_obj(f"{label}_ObsidianChestPlate", (height * 0.090, 0, height * 0.665), (height * 0.016, shoulder * 1.35, height * 0.125), armor), armature, "chest", objects)
    ...
    add_assigned(add_box_obj(f"{label}_Back_ObsidianMantlePlate", (-height * 0.035, 0, height * 0.69), (height * 0.020, shoulder * 1.12, height * 0.095), armor), armature, "chest", objects)
    ...
    for side, sign in (("L", 1), ("R", -1)):
        ...
        add_assigned(add_box_obj(f"{label}_BonePauldron_{side}", (height * 0.018, sign * shoulder * 1.12, height * 0.745), (height * 0.045, height * 0.074, height * 0.030), bone), armature, "chest", objects)
        add_assigned(add_cone_between(f"{label}_PauldronHorn_{side}", (height * 0.030, sign * shoulder * 1.38, height * 0.765), (height * 0.055, sign * shoulder * 1.86, height * 0.790), height * 0.010, 0.0, bone, 8), armature, "chest", objects)
    ...
    sockets = [
        ...
        ("SOCKET_vfx_sorcerer_focus", (height * 0.120, 0, height * 0.92), "head"),
        ("SOCKET_vfx_worthiness_mark", (height * 0.115, 0, height * 0.49), "pelvis"),
...
def export_asset() -> None:
    clear_scene()
    setup_scene()
    materials = create_materials(INF_MAGE_MATERIALS)
    armature, objects = add_infernal_mage(ASSET_NAME, f"SKEL_{ASSET_NAME}", materials)
    add_asset_metadata(
        ASSET_NAME,
        "First-pass Infernal Mage class overlay with horn crown, wing mantle, open claw casting, brand sockets, skull belt, and ritual cloth; final sculpt, retopo, skinning, physics, LODs, animation, and textures pending",
...
def render_review() -> None:
    ...
    add_infernal_mage(ASSET_NAME, f"SKEL_{ASSET_NAME}", materials)
    ...
    add_asset_metadata(
        ASSET_NAME,
        "Approved Infernal Mage first-pass DCC review scene with 203 cm Standard caster, 180 cm humanoid marker, and 244 cm Greater-band marker",
        "/Game/Aerathea/Characters/Infernals/Mage/SK_INF_Mage_A01",
    )

    bpy.ops.object.light_add(type="AREA", location=(520, -360, 430))
    key = bpy.context.object
    key.name = "AET_InfernalMageReview_KeyLight"
    key.data.energy = 7200
    key.data.size = 560
    bpy.ops.object.light_add(type="AREA", location=(380, 350, 300))
    fill = bpy.context.object
    fill.name = "AET_InfernalMageReview_FillLight"
    fill.data.energy = 3400
    fill.data.size = 700
    bpy.ops.object.light_add(type="SUN", location=(0, 0, 500), rotation=(math.radians(52), 0, math.radians(35)))
    sun = bpy.context.object
    sun.name = "AET_InfernalMageReview_SunFill"
    sun.data.energy = 1.15

    bpy.ops.object.camera_add(location=(620, -500, 235))
    camera = bpy.context.object
    target = Vector((10, 0, 118))
