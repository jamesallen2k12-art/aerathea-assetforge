#!/usr/bin/env python3
"""Build A1 Blood Axe cairn as a cleaned multi-view relief candidate.

Run with:
    blender --background --python Tools/DCC/build_bloodaxe_cairn_a1_clean_multiview_relief.py

This pass removes the marker/label artifacts from the approved A1 turnaround
views and projects those cleaned views onto shallow orthographic relief shells.
It is a DCC source candidate for static-prop visual testing, not a final sculpt
or fully game-ready asset until Unreal import and Flamestrike approval.
"""
...
ASSET_NAME = "SM_GIA_BloodAxeCairnSlabCluster_A01_CleanMultiViewRelief"
...
def marker_pixel(red: int, green: int, blue: int) -> bool:
    chroma = max(red, green, blue) - min(red, green, blue)
    luma = (red + green + blue) / 3.0
    return chroma > 82 and luma > 112 and max(red, green, blue) > 170


def replace_markers(crop: Image.Image) -> Image.Image:
    cleaned = crop.copy().convert("RGBA")
    median = crop.filter(ImageFilter.MedianFilter(17)).convert("RGBA")
    ...
    for y in range(crop.height):
        for x in range(crop.width):
            ...
            if marker_pixel(red, green, blue):
                mask_pixels[x, y] = 255
    mask = mask.filter(ImageFilter.MaxFilter(13)).filter(ImageFilter.GaussianBlur(1.4))
    cleaned.paste(median, (0, 0), mask)
    return cleaned


def masked_crop(source: Image.Image, spec: ViewSpec) -> Path:
    ensure_dir(TEXTURE_DIR)
    crop = replace_markers(source.crop(spec.box).convert("RGBA"))
    ...
    crop.putalpha(alpha)
    path = TEXTURE_DIR / f"T_GIA_BloodAxeCairnSlabCluster_A01_CleanMultiViewRelief_{spec.name}_BC.png"
    crop.save(path)
    return path
...
def build_review_board() -> None:
    ...
    draw.text((52, 32), "A01 Clean multi-view relief DCC candidate", fill=(25, 23, 20, 255), font=font(32))
    draw.text((52, 78), "Purpose: marker-cleaned concept fidelity check from approved front/right/back/top views.", fill=(58, 52, 45, 255), font=font(21))
    ...
    draw.text((900, 450), "DCC Front relief", fill=(40, 36, 32, 255), font=font(20))
    draw.text((1420, 450), "DCC Right relief", fill=(40, 36, 32, 255), font=font(20))
    draw.text((900, 920), "DCC Back relief", fill=(40, 36, 32, 255), font=font(20))
    draw.text((1420, 920), "DCC Top relief", fill=(40, 36, 32, 255), font=font(20))
...
def build() -> None:
    ...
    materials = {name: make_projection_material(f"M_GIA_BloodAxeCairn_A01_CleanMultiViewRelief_{name}", path) for name, path in textures.items()}
    side_material = make_solid_material("M_GIA_BloodAxeCairn_A01_CleanMultiViewRelief_SideStone", (0.42, 0.39, 0.32, 1.0))
    ...
    add_asset_metadata(
        ASSET_NAME,
        "A01 clean multi-view relief candidate using marker-cleaned approved turnaround front/right/back/left/top panels. DCC source candidate only; not fully game-ready or canon.",
        UNREAL_PATH,
    )
