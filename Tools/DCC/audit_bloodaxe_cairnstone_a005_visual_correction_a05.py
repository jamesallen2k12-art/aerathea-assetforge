#!/usr/bin/env python3
"""Independently audit the A005 A05 measured oval-base correction."""
...
VALIDATION_REL = BLUEPRINT_ROOT / "manifests/VISUAL_CORRECTION_A05_VALIDATION.json"
DCC_ROOT = Path("SourceAssets/Blender/Props/Giants/BloodAxe/Cairns") / ASSET
A05_BLEND_REL = DCC_ROOT / f"{ASSET}_DCCGameReady_VisualCorrection_A05.blend"
...
OUTPUT_ROOT_REL = Path("Saved/Automation/DCC") / ASSET / "Production/VisualCorrection_A05"
FINAL_REL = OUTPUT_ROOT_REL / f"{ASSET}_FINAL_CORRECTED_3D_A05.png"
FINAL_RGBA_REL = OUTPUT_ROOT_REL / f"{ASSET}_FINAL_CORRECTED_3D_A05_OBJECT_RGBA.png"
FINAL_AUDIT_REL = OUTPUT_ROOT_REL / "FINAL_RENDER_AUDIT_A05.json"
...
A05_BC_REL = Path("SourceAssets/Textures/Props/Giants/BloodAxe/Cairns") / ASSET / "VisualCorrection_A05/T_GIA_BloodAxeCairnstone_A005_VF_A05_BC.png"
...
def source_owned_rgb_audit() -> Dict[str, Any]:
    ...
    candidate = Image.open(ROOT / A05_BC_REL).convert("RGB")
    compared = 0
    mismatches = 0
...
def main() -> int:
    ...
    candidate_lods = {
        lod_name: load_object_from_blend(bpy, A05_BLEND_REL, f"{ASSET}_{lod_name}")
    ...
    plinth_signatures = {
        ...
        "A05": component_signature(lod0, plinth["indices"]),
    }
    ...
    for name, record, footprint in (
        ("C002", upper, (119.097222, 77.647059)),
        ("C003", lower, (134.652778, 96.561086)),
    ...
    collisions = []
    with bpy.data.libraries.load(str(ROOT / A05_BLEND_REL), link=False) as (data_from, data_to):
    ...
    footprint_pass = close(upper["dimensions"][:2], [119.097222, 77.647059], 0.01) and close(lower["dimensions"][:2], [134.652778, 96.561086], 0.01) and close(apron["dimensions"][:2], [140.0, 110.0], 0.01)
    ...
    gate("G08_exact_A04_plinth_geometry_and_uv", plinth_signatures["A04"] == plinth_signatures["A05"], plinth_signatures)
    gate("G09_material_and_uv", all(lods[name]["materials"] == 1 and lods[name]["uv_layers"] == ["UVMap", "LightmapUV"] for name in lods) and all(name in material_nodes for name in ("A05_DIRECT_SOURCE_BASECOLOR", "A05_DIRECTX_NORMAL", "A05_ORM_NONMETALLIC")), {"lods": {name: {"materials": lods[name]["materials"], "uv_layers": lods[name]["uv_layers"]} for name in lods}, "material_nodes": material_nodes})
    ...
    gate("G13_source_owned_rgb_exact", source_rgb["pixels_compared"] > 0 and source_rgb["mismatches"] == 0 and measurement["validation"]["top_witness_samples_checked"] == 96 and measurement["validation"]["top_witness_rgb_mismatches"] == 0, {"atlas": source_rgb, "measurement_witnesses": measurement["validation"]})
    ...
    status = (
        ...
        if complete
        else "blocked_a05_independent_audit"
    )
    validation = {
        "schema": "aerathea.a005_visual_correction_a05_validation.v1",
        "asset_id": ASSET,
