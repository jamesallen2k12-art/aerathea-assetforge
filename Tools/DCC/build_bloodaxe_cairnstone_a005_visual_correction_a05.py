#!/usr/bin/env python3
"""Build the A005 A05 measured oval replacement-base candidate."""
...
PLAN_REL = BLUEPRINT_ROOT / "manifests/VISUAL_CORRECTION_A05_PLAN.json"
MEASUREMENT_REL = BLUEPRINT_ROOT / "manifests/VISUAL_CORRECTION_A05_MEASUREMENT_AUDIT.json"
...
TEXTURE_ROOT = TEXTURE_PARENT / "VisualCorrection_A05"
BC_REL = TEXTURE_ROOT / "T_GIA_BloodAxeCairnstone_A005_VF_A05_BC.png"
N_REL = TEXTURE_ROOT / "T_GIA_BloodAxeCairnstone_A005_VF_A05_N.png"
ORM_REL = TEXTURE_ROOT / "T_GIA_BloodAxeCairnstone_A005_VF_A05_ORM.png"
DCC_ROOT = Path("SourceAssets/Blender/Props/Giants/BloodAxe/Cairns") / ASSET
BLEND_REL = DCC_ROOT / f"{ASSET}_DCCGameReady_VisualCorrection_A05.blend"
MANIFEST_REL = DCC_ROOT / f"{ASSET}_VISUAL_CORRECTION_A05_MANIFEST.json"
EXPORT_ROOT = Path("SourceAssets/Exports/Props/Giants/BloodAxe/Cairns") / ASSET / "VisualCorrection_A05"
FBX_RELS = {
    "LOD0": EXPORT_ROOT / f"{ASSET}_VisualCorrection_A05.fbx",
    "LOD1": EXPORT_ROOT / f"{ASSET}_VisualCorrection_A05_LOD1.fbx",
    "LOD2": EXPORT_ROOT / f"{ASSET}_VisualCorrection_A05_LOD2.fbx",
    "LOD3": EXPORT_ROOT / f"{ASSET}_VisualCorrection_A05_LOD3.fbx",
}
...
def source_panel_y_a05(view: str, z_value: float) -> float:
    mappings = {
...
def base_half_extent_a05(z_value: float, axis: str) -> float:
    if z_value <= 10.5:
        outer = 70.0 if axis == "x" else 55.0
        inner = 67.326389 if axis == "x" else 48.280543
        return outer + (inner - outer) * max(0.0, z_value) / 10.5
    if z_value <= 25.5:
        lower = 67.326389 if axis == "x" else 48.280543
        upper = 63.4375 if axis == "x" else 46.0
        return lower + (upper - lower) * (z_value - 10.5) / 15.0
    lower = 59.548611 if axis == "x" else 38.823529
    upper = 56.145833 if axis == "x" else 36.5
...
def configure_base(base: Any, a04: Any) -> None:
    ...
    def loft_a05(
        ...
        if name == "LOWER_COURSE_CORE":
            result = masonry_course_shell(
                ...
                6,
                (
                    (10.0, 126.0, 90.0, "hidden"),
                    (10.5, 134.652778, 96.561086, "side"),
                    (12.2, 134.652778, 96.561086, "side"),
                    (24.2, 126.875, 92.0, "side"),
                    (25.5, 126.875, 92.0, "top"),
                    (25.5, 119.097222, 77.647059, "top"),
                ),
                70503,
                collection,
                joint_depth=4.1,
        ...
        if name == "UPPER_COURSE_CORE":
            result = masonry_course_shell(
                ...
                6,
                (
                    (25.0, 110.0, 70.0, "hidden"),
                    (25.5, 119.097222, 77.647059, "side"),
                    (27.0, 119.097222, 77.647059, "side"),
                    (33.8, 112.291667, 74.0, "side"),
                    (35.0, 112.291667, 74.0, "top"),
                    (35.0, 96.0, 68.0, "top"),
                ),
                70502,
                collection,
    ...
    def project_point_a05(point: Vec3, view: str, bbox: Sequence[int], row_spans: Sequence[Tuple[int, int]]):
        ...
        if view == "top":
            ...
            if z_value >= 25.0:
                ...
                half_px_x, half_px_y = 83.5, 91.5
                half_cm_x, half_cm_y = 119.097222 * 0.5, 77.647059 * 0.5
                return (
            ...
            if z_value >= 10.0:
                ...
                half_px_x, half_px_y = 94.0, 103.5
                half_cm_x, half_cm_y = 134.652778 * 0.5, 96.561086 * 0.5
                return (
                    center_x + max(-1.0, min(1.0, x_value / half_cm_x)) * half_px_x,
                    center_y - max(-1.0, min(1.0, y_value / half_cm_y)) * half_px_y,
                )
        ...
        if view in ("front", "back", "left", "right"):
            panel_y = source_panel_y_a05(view, max(0.0, min(220.0, z_value))) if z_value < 34.0 else a04.source_panel_y(view, z_value)
            horizontal = {"front": x_value, "back": -x_value, "left": -y_value, "right": y_value}[view]
            axis = "x" if view in ("front", "back") else "y"
            half_extent = a04.interpolate_profile(z_value, axis) if z_value >= 34.0 else base_half_extent_a05(z_value, axis)
    ...
    def create_materials_a05(bpy: Any):
        stone, removable = a04_materials(bpy)
        stone.name = "M_GIA_BloodAxeCairnstone_A005_VisualCorrection_A05"
        removable.name = "M_GIA_BloodAxeCairnstone_A005_A05_REMOVED_HELPER"
        for node in stone.node_tree.nodes:
            if node.name == "A04_DIRECT_SOURCE_BASECOLOR":
                node.name = "A05_DIRECT_SOURCE_BASECOLOR"
            elif node.name == "A04_DIRECTX_NORMAL":
                node.name = "A05_DIRECTX_NORMAL"
            elif node.name == "A04_ORM_NONMETALLIC":
                node.name = "A05_ORM_NONMETALLIC"
        stone["aerathea_display_color_chain"] = "A05 component-local exact source RGB; no gamma or grading"
        return stone, removable

    def assign_uv_a05(obj: Any) -> Dict[str, int]:
        # Capture A05's source-panel routing for the replacement base, then
        ...
        if source_layer is None:
            raise RuntimeError("A05 source UVMap missing")
        ...
        if layer is None:
            raise RuntimeError("A05 UVMap missing")
        ...
        # Restore A05 source-panel ownership only on the three replacement
        ...
        obj["aerathea_A05_structure"] = "preserved_A04_plinth_on_measured_oval_upper_and_lower_masonry_courses"
        obj["aerathea_A05_component_footprints_cm"] = json.dumps(
            {"C002": [119.097222, 77.647059], "C003": [134.652778, 96.561086], "C004": [140.0, 110.0]}
        )
        obj["aerathea_A05_component_local_top_uv"] = True
        obj["aerathea_A05_C004_authored_stone_faces"] = c004_authored_faces
        obj["aerathea_A05_color_grading"] = False
        return counts

    base.loft = loft_a05
    base.project_point = project_point_a05
    base.create_materials = create_materials_a05
    base.assign_source_uv = assign_uv_a05
...
def schema_report() -> Dict[str, Any]:
    ...
    passed = all(item["pass"] for item in checks.values()) and measurement["decision"] == "authoritative measurement record ready for A05 contract"
    return {
        "schema": "aerathea.a005_visual_correction_a05_builder_preflight.v1",
        "asset_id": ASSET,
...
def build() -> Dict[str, Any]:
    ...
    if preflight["status"] != "pass_schema_only_no_output":
        raise RuntimeError("A05 authority preflight blocked")
    base = load_module("a005_a05_packaging", BASE_MODULE_PATH)
    ...
    manifest.update(
        {
            "schema": "aerathea.a005_visual_correction_a05_candidate.v1",
            "contract_id": CONTRACT,
            "date": "2026-07-21",
            "status": "candidate_pending_a05_independent_audit",
            "artifact_classification": "candidate",
            "pipeline_status": "DCC game-ready candidate pending A05 audit and Flamestrike review",
            "recovery_from": "A04 visually rejected rectangular cumulative base",
            "geometry_authority": "A05 measured owner-view extents plus explicitly classified n=2 oval construction; A04 base geometry inputs zero",
            ...
            "a05_structure": {
                ...
                "C002": {"role": "upper measured oval masonry course", "footprint_cm": [119.097222, 77.647059], "height_cm": 9.5, "blocks": 17, "oval_exponent": 2.0},
                "C003": {"role": "larger lower measured oval masonry course", "footprint_cm": [134.652778, 96.561086], "height_cm": 15.0, "blocks": 19, "oval_exponent": 2.0},
